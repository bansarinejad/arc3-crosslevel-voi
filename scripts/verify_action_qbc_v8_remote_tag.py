"""One-shot bounded Windows verification of the frozen action-QBC v8 Git tag."""
...
_SCHEMA_VERSION: Final = "action-qbc-v8-remote-tag-verification-receipt-v1"
...
_CLAIM_PATH: Final = (
    r"D:\kaggle competitions\arc3-crosslevel-voi-action-qbc-v8-remote-verification-claim-v4.json"
)
_START_CLAIM_PATH: Final = (
    r"D:\kaggle competitions\arc3-crosslevel-voi-action-qbc-v8-remote-verifier-start-claim-v4.json"
)
_RECEIPT_PATH: Final = (
    r"D:\kaggle competitions\arc3-crosslevel-voi-action-qbc-v8-remote-verification-v4.json"
)
...
_VERIFIER_CLEANUP_ENDPOINT_SECONDS: Final = 420
...
def _windows_repository_contract() -> dict[str, Any]:
    return {
        ...
        "common_directory": (
            r"D:\kaggle competitions\arc3-crosslevel-voi\.git"
        ),
        "forbidden_admin_relative_paths": [
            r".git\commondir",
        ...
        "local_config": {
            "branch.action-qbc-v6-prereg.merge": (
                "refs/heads/action-qbc-v6-prereg"
            ),
            ...
            "branch.action-qbc-v8-prereg.merge": (
...
class _ProtocolFailure(RuntimeError):
    """A remote-verification identity or lifecycle invariant failed closed."""
...
def canonical_json_bytes(value: object) -> bytes:
    """Return the sole sorted, compact, finite, ASCII JSON representation."""
...
def _manifest_entry(registration: Mapping[str, Any], path: str) -> dict[str, Any]:
    ...
    for collection_name in ("preregistration_tree", "open_freeze_added_files"):
        ...
        if not isinstance(rows, list):
            _fail(f"registration {collection_name} manifest is not a list")
        checked: list[dict[str, Any]] = []
        observed_paths: list[str] = []
        for index, row_value in enumerate(rows):
            if not isinstance(row_value, dict):
                _fail(f"registration {collection_name} manifest row {index} is not an object")
            row = row_value
            _require_keys(
                row,
            ...
            row_path = row["path"]
            if row["mode"] != "100644" or not isinstance(row_path, str) or not row_path:
                _fail(f"registration {collection_name} manifest path is invalid")
            observed_paths.append(row_path)
            _require_lower_hex(row["git_blob_sha1"], 40, "manifest Git blob")
            _require_lower_hex(row["sha256"], 64, "manifest SHA-256")
            _require_int(row["byte_count"], "manifest byte_count")
            checked.append(row)
        if observed_paths != sorted(
            observed_paths,
            key=lambda item: item.encode("utf-8"),
        ) or len(set(observed_paths)) != len(observed_paths):
            _fail(f"registration {collection_name} manifest is not uniquely path sorted")
        validated[collection_name] = checked
    matches = [
        row
        for row in validated["open_freeze_added_files"]
        if row["path"] == path
    ]
    if len(matches) != 1:
...
def _load_registration(root: Path, path: Path) -> _Registration:
    data = _read_plain_file(path, "registration")
    registration = _parse_canonical_object(data, "registration")
    ...
    if registration["content_sha256"] != canonical_sha256(content):
    ...
    if not isinstance(execution_value, dict):
        _fail("registration execution_contract is invalid")
    ...
    if windows_contract != _windows_repository_contract():
        _fail("registration Windows repository contract is invalid")
    if execution["remote_policy"] != _remote_policy():
        _fail("registration remote policy is invalid")
    if (
        execution["local_git_timeout_seconds"]
        != _REGISTERED_LOCAL_GIT_TIMEOUT_SECONDS
    ):
        _fail("registration local Git timeout is invalid")
    if execution["remote_supervisor_argv"] != _expected_supervisor_argv():
        _fail("registration supervisor argv is invalid")
    if execution["remote_verifier_argv"] != _expected_verifier_argv():
        _fail("registration verifier argv is invalid")
    if (
        execution["remote_claim_windows_path"] != _CLAIM_PATH
        or execution["remote_verifier_claim_windows_path"] != _START_CLAIM_PATH
        or execution["remote_receipt_windows_path"] != _RECEIPT_PATH
        or execution["remote_supervisor_receipt_windows_path"]
        != _SUPERVISOR_RECEIPT_PATH
    ):
        _fail("registration Windows remote paths are invalid")
    argv_hashes = execution["argv_hashes"]
    if not isinstance(argv_hashes, dict):
        _fail("registration argv_hashes is invalid")
    _require_keys(argv_hashes, _ARGV_HASH_KEYS, "argv_hashes")
    for name, digest in argv_hashes.items():
        _require_lower_hex(digest, 64, f"argv_hashes.{name}")
    if argv_hashes["remote_supervisor"] != canonical_sha256(_expected_supervisor_argv()):
        _fail("registration supervisor argv hash is invalid")
    if argv_hashes["remote_verifier"] != canonical_sha256(_expected_verifier_argv()):
        _fail("registration verifier argv hash is invalid")

    supervisor_manifest = _manifest_entry(registration, _SUPERVISOR_SCRIPT)
    verifier_manifest = _manifest_entry(registration, _VERIFIER_SCRIPT)
    for script, entry in (
        (_SUPERVISOR_SCRIPT, supervisor_manifest),
        (_VERIFIER_SCRIPT, verifier_manifest),
    ):
        raw = _read_plain_file(root / Path(script), script)
        if (
            len(raw) != entry["byte_count"]
            or hashlib.sha256(raw).hexdigest() != entry["sha256"]
            or _git_blob_sha1(raw) != entry["git_blob_sha1"]
        ):
            _fail(f"raw script identity differs from registration: {script}")
...
def _tool_object(path_text: str, version: str, expected_sha256: str, name: str) -> dict[str, str]:
    path = Path(path_text)
    observed = _sha256_file(path, name)
    if observed != expected_sha256:
        _fail(f"{name} executable SHA-256 is invalid")
    return {"path": path_text, "version": version, "sha256": expected_sha256}
...
def _validate_tools() -> tuple[dict[str, str], dict[str, str], dict[str, str]]:
    if os.path.normcase(os.path.abspath(sys.executable)) != os.path.normcase(_PYTHON_PATH):
        _fail("verifier Python executable differs from registration")
    if sys.implementation.name != "cpython" or sys.version_info[:3] != (3, 12, 3):
        _fail("verifier Python version differs from registration")
    python = _tool_object(_PYTHON_PATH, _PYTHON_VERSION, _PYTHON_SHA256, "Python")
    git = _tool_object(_GIT_PATH, _GIT_VERSION, _GIT_SHA256, "Git")
    taskkill = _tool_object(_TASKKILL_PATH, _TASKKILL_VERSION, _TASKKILL_SHA256, "taskkill")
    return python, git, taskkill
...
def _validate_lifecycle_claim(
    ...
    claim = _parse_canonical_object(data, "lifecycle claim")
    _require_keys(claim, _CLAIM_KEYS, "lifecycle claim")
    _require_lower_hex(claim["open_freeze_commit_sha"], 40, "open-freeze commit")
    if (
        claim["schema_version"] != _LIFECYCLE_CLAIM_SCHEMA
        or claim["treatment_id"] != _TREATMENT_ID
        or claim["open_freeze_tag"] != _OPEN_FREEZE_TAG
        or claim["registration_content_sha256"]
        != registration.value["content_sha256"]
        or claim["supervisor_argv_sha256"]
        != registration.execution["argv_hashes"]["remote_supervisor"]
        or claim["supervisor_script_git_blob_sha1"]
        != registration.supervisor_manifest["git_blob_sha1"]
        or claim["supervisor_script_sha256"]
        != registration.supervisor_manifest["sha256"]
        or claim["verifier_script_git_blob_sha1"]
        != registration.verifier_manifest["git_blob_sha1"]
        or claim["verifier_script_sha256"] != registration.verifier_manifest["sha256"]
    ):
        _fail("lifecycle claim does not match registration")
    return claim, hashlib.sha256(data).hexdigest()


def _start_claim_object(
    registration: _Registration,
...
def _validate_attempt(
    ...
    if classification == "verified":
        if exit_code != 0 or stdout != expected_stdout or value["timed_out"] is not False:
            _fail("verified attempt evidence is invalid")
    elif classification == "retryable_empty_exit_0":
        if exit_code != 0 or stdout or value["timed_out"] is not False:
        ...
        if value["timed_out"] and exit_code != _SYNTHETIC_TIMEOUT_EXIT:
        ...
        if stdout or stderr or value["timed_out"] is not False:
...
def _validate_receipt(
    ...
    *,
    ...
    _require_keys(value, _RECEIPT_KEYS, "remote receipt")
    expected_stdout = (
...
def _publish_canonical(
    ...
    try:
        ...
        raise _ProtocolFailure(f"{purpose} parent is unavailable") from exc
    ...
    try:
        ...
        stage_meta = stage.stat(follow_symlinks=False)
        stage_identity = (stage_meta.st_dev, stage_meta.st_ino)
        ...
        if (final_meta.st_dev, final_meta.st_ino) != stage_identity:
            _fail(f"published {purpose} does not share the staged inode")
        final_data = _read_plain_file(path, f"published {purpose}")
        if (
            final_data != data
            or hashlib.sha256(final_data).digest() != hashlib.sha256(data).digest()
        ):
            _fail(f"published {purpose} bytes changed")
        ...
        if stage_identity is not None:
            with contextlib.suppress(OSError):
                observed_meta = stage.stat(follow_symlinks=False)
                if (observed_meta.st_dev, observed_meta.st_ino) == stage_identity:
                    stage.unlink()
...
def _run_bounded_process(
    ...
    *,
    ...
    stderr_cap: int,
    deadline_reason: str,
    duration_start_ns: int | None = None,
) -> _ManagedResult:
    observed_start_ns = time.monotonic_ns()
    started_ns = observed_start_ns if duration_start_ns is None else duration_start_ns
    if started_ns > observed_start_ns:
        _fail("managed-process duration epoch is in the future")
    if observed_start_ns >= live_deadline_ns:
        duration = max(0, (observed_start_ns - started_ns) // 1_000_000)
    ...
    stdout_thread = threading.Thread(
        ...
        name="arc3-v8-stdout-capture",
        daemon=True,
    )
    stderr_thread = threading.Thread(
        target=_capture_stream,
        args=(process.stderr, stderr_capture),
        name="arc3-v8-stderr-capture",
        daemon=True,
    )
    stdout_thread.start()
    stderr_thread.start()
    ...
    if controller_triggered:
        cleanup_passes = _cleanup_tree(
            process, job, deadline_ns=operation_deadline_ns
        )
    else:
        try:
            active = job.active_processes()
...
def _attempt_record(
    ...
    *,
    ...
    if not result.spawned:
        ...
        classification = (
            "verified"
            if result.exit_code == 0 and result.stdout == expected_stdout
            else "unexpected_output"
        )
    elif result.exit_code == 0:
        classification = "retryable_empty_exit_0"
    elif result.exit_code == 128:
        classification = "retryable_git_128"
    else:
...
def _run_git_command(
    ...
    *,
    ...
    if now + required > overall_deadline_ns:
        _fail("insufficient verifier budget for local Git identity validation")
...
def _validate_repository_identity(
    ...
    *,
    ...
    if head != f"{expected_commit}\n".encode("ascii"):
        _fail("local HEAD differs from the lifecycle claim")
    ...
    if (
        tag_type != b"commit\n"
        or tag_commit != f"{expected_commit}\n".encode("ascii")
        or peeled_commit != tag_commit
    ):
        _fail("local open-freeze tag is not the expected lightweight tag")
    ...
    delta = _run_git_command(
        ["-C", _REPOSITORY_ROOT, "diff", "--name-status", "-z", parent, expected_commit],
        overall_deadline_ns=overall_deadline_ns,
    )
    ...
    for path in (_SUPERVISOR_SCRIPT, _VERIFIER_SCRIPT, _REGISTRATION_PATH):
        ...
        tree_oid = _tree_blob_oid(path, expected_commit, overall_deadline_ns=overall_deadline_ns)
...
def _remote_attempt(
    index: int,
    *,
    live_admission_deadline_ns: int,
    cleanup_deadline_ns: int,
    expected_stdout: bytes,
) -> dict[str, Any] | None:
    now = time.monotonic_ns()
    if (
        now + _ATTEMPT_TIMEOUT_SECONDS * 1_000_000_000
        > live_admission_deadline_ns
        or now
        + (_ATTEMPT_TIMEOUT_SECONDS + _CHILD_CLEANUP_TIMEOUT_SECONDS)
        * 1_000_000_000
        > cleanup_deadline_ns
    ):
        return None
    attempt_deadline_ns = now + _ATTEMPT_TIMEOUT_SECONDS * 1_000_000_000
    live_deadline_ns = min(attempt_deadline_ns, live_admission_deadline_ns)
    deadline_reason = (
        "timeout"
        if attempt_deadline_ns < live_admission_deadline_ns
    ...
    result = _run_bounded_process(
        ...
        cwd=_NEUTRAL_GIT_CWD,
        environment=_git_environment(),
        live_deadline_ns=live_deadline_ns,
    ...
    return _attempt_record(index, result, expected_stdout=expected_stdout)


def _sleep_retry(prior_end_ns: int, overall_deadline_ns: int) -> bool:
    target = prior_end_ns + _RETRY_DELAY_SECONDS * 1_000_000_000
    if target > overall_deadline_ns:
        return False
    ...
    *,
    ...
    for index in range(1, _MAX_ATTEMPTS + 1):
        attempt = _remote_attempt(
            index,
            live_admission_deadline_ns=live_admission_deadline_ns,
            cleanup_deadline_ns=cleanup_deadline_ns,
        ...
        if classification == "verified":
            selected = index
            break
        if classification not in _RETRYABLE_CLASSIFICATIONS or index == _MAX_ATTEMPTS:
            break
        prior_end = time.monotonic_ns()
        if not _sleep_retry(prior_end, live_admission_deadline_ns):
            break
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--attempt-timeout-seconds", required=True, type=int)
    parser.add_argument("--retry-delay-seconds", required=True, type=int)
    parser.add_argument("--overall-deadline-seconds", required=True, type=int)
    return parser


def _validate_invocation(args: argparse.Namespace, observed_argv: Sequence[str]) -> Path:
    if os.name != "nt":
        _fail("remote tag verification requires Windows")
    if sys.flags.isolated != 1 or sys.flags.dont_write_bytecode != 1:
        _fail("remote tag verification requires Python -I -B")
    if [sys.executable, "-I", "-B", *observed_argv] != _expected_verifier_argv():
        _fail("verifier argv differs from the registered command")
    if (
        args.repository_root != "."
        or args.registration != _REGISTRATION_ARGV_PATH
        or args.claim != _CLAIM_PATH
        or args.verifier_start_claim != _START_CLAIM_PATH
        or args.receipt != _RECEIPT_PATH
        or args.git_executable != _GIT_PATH
        or args.taskkill_executable != _TASKKILL_PATH
        or args.max_attempts != _MAX_ATTEMPTS
        or args.attempt_timeout_seconds != _ATTEMPT_TIMEOUT_SECONDS
        or args.retry_delay_seconds != _RETRY_DELAY_SECONDS
        or args.overall_deadline_seconds != _OVERALL_DEADLINE_SECONDS
    ):
        _fail("verifier arguments differ from registration")
    root = Path(os.path.abspath(os.getcwd()))
    if os.path.normcase(str(root)) != os.path.normcase(_REPOSITORY_ROOT):
        _fail("verifier cwd differs from the registered repository")
    expected_script = root / Path(_VERIFIER_SCRIPT)
    if os.path.normcase(os.path.abspath(sys.argv[0])) != os.path.normcase(
        str(expected_script)
    ):
        _fail("verifier script origin differs from registration")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    observed_argv = list(sys.argv) if argv is None else [_VERIFIER_SCRIPT, *argv]
    root = _validate_invocation(args, observed_argv)
    registration_path = root / Path(_REGISTRATION_PATH)
    registration = _load_registration(root, registration_path)
    python, git, taskkill = _validate_tools()
    lifecycle_data = _read_plain_file(Path(_CLAIM_PATH), "lifecycle claim")
    lifecycle_claim, lifecycle_claim_sha256 = _validate_lifecycle_claim(
        lifecycle_data,
        registration,
    )
    ...
    if start_path.exists() or start_path.is_symlink():
        _fail("verifier-start claim already exists")
    if receipt_path.exists() or receipt_path.is_symlink():
        _fail("remote receipt already exists")
    start_claim = _start_claim_object(registration, lifecycle_claim, lifecycle_claim_sha256)
    start_data = _publish_canonical(
        start_path,
        start_claim,
        lambda value: _validate_start_claim(value, start_claim),
        "verifier-start-claim",
    )
    start_claim_sha256 = hashlib.sha256(start_data).hexdigest()

    attempts: list[dict[str, Any]] = []
    selected: int | None = None
    overall_deadline_ns = started_ns + _OVERALL_DEADLINE_SECONDS * 1_000_000_000
    cleanup_deadline_ns = (
        started_ns + _VERIFIER_CLEANUP_ENDPOINT_SECONDS * 1_000_000_000
    )
    receipt_target_ns = (
        started_ns + _VERIFIER_CHILD_DEADLINE_SECONDS * 1_000_000_000
    )
    try:
        # The start claim is an authorized sibling of the repository.  Publish
        # and durably validate it before opening the frozen before/after interval
        # that includes the repository ancestor chain.
        ...
        _validate_repository_identity(
            root,
            registration,
            lifecycle_claim["open_freeze_commit_sha"],
            overall_deadline_ns=overall_deadline_ns,
        ...
        attempts, selected = _run_attempts(
            live_admission_deadline_ns=overall_deadline_ns,
            cleanup_deadline_ns=cleanup_deadline_ns,
            expected_stdout=expected_stdout,
        )
    except Exception:
    ...
    total_milliseconds = max(0, (time.monotonic_ns() - started_ns) // 1_000_000)
    receipt = _receipt_object(
        ...
        total_duration_milliseconds=total_milliseconds,
    )
    if time.monotonic_ns() >= receipt_target_ns:
        _fail("verifier receipt target elapsed before publication")
    _publish_canonical(
        receipt_path,
        receipt,
        lambda value: _validate_receipt(
            value,
            registration=registration,
            lifecycle_claim=lifecycle_claim,
            lifecycle_claim_sha256=lifecycle_claim_sha256,
