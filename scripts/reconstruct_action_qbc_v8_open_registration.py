...
PREREGISTRATION_TAG: Final = "prereg-action-qbc-v8-open-bounded-remote-verification-v6"
PREREGISTRATION_COMMIT: Final = "61cebe90a2f4f7c78ec45119de53a482ed13a655"
PREREGISTRATION_TREE: Final = "65695876c44eeb8cac5437149384071f88ff6018"
...
PREREGISTRATION_DOCUMENT_BLOB: Final = "5e870ed0bbbff6fcb4352f6e914d870254773f68"
PREREGISTRATION_DOCUMENT_SHA256: Final = (
    "0ba4cc55ca2b31433bc458972ffc32d87f84b610673fa22ed2b4dd4a8bfc1a41"
)
PREREGISTRATION_DOCUMENT_BYTE_COUNT: Final = 32_370
...
OPEN_FREEZE_TAG: Final = "action-qbc-v8-open-diagnostic-freeze-v3"
RESULT_TAG: Final = "action-qbc-v8-open-diagnostic-result-v3"
...
def _resolve_phase(root: Path) -> tuple[str, str, list[tuple[str, str, str, int | None]]]:
    ...
    _require_direct_child(root, PREREGISTRATION_COMMIT, OPEN_FREEZE_V2_COMMIT, "P8v6")
    p8_v6_delta = _git(
        root,
        "diff",
        "--name-status",
    ...
    if (
        ...
        or p8_v6_entry.get("git_blob_sha1") != PREREGISTRATION_DOCUMENT_BLOB
        or hashlib.sha256(p8_v6_raw).hexdigest() != PREREGISTRATION_DOCUMENT_SHA256
        or len(p8_v6_raw) != PREREGISTRATION_DOCUMENT_BYTE_COUNT
    ):
    ...
    expected_net_addition = b"A\0" + PREREGISTRATION_DOCUMENT.encode("utf-8") + b"\0"
    if p8_v5_to_v6 != expected_net_addition:
        raise ReconstructionError("P8v5..P8v6 is not the one-document net addition")
    p8_v6_tree = _tree_entries(root, PREREGISTRATION_COMMIT)
    p8_v6_document_entry = next(
        (entry for entry in p8_v6_tree if entry[0] == PREREGISTRATION_DOCUMENT),
        None,
    )
    ...
    if p8_v6_tree_sha != PREREGISTRATION_TREE:
    ...
    if head == PREREGISTRATION_COMMIT:
        if _git(root, "diff", "--name-status", "--no-renames", "-z") != b"":
            raise ReconstructionError("pre-O8v3 worktree has filtered changes to staged/tracked bytes")
        cached = _git(
        ...
        if cached not in {
            ...
            raise ReconstructionError("pre-O8v3 index is not the exact fourteen- or fifteen-addition allowlist")
        ...
        for record in status.split(b"\0"):
            ...
            if len(record) < 4 or record[2:3] != b" ":
                raise ReconstructionError("malformed pre-O8v3 Git status")
            code = record[:2]
            path = record[3:].decode("utf-8")
            if path not in allowed or code not in {b"A ", b"??"}:
                raise ReconstructionError(f"non-allowlisted pre-O8v3 state: {code!r} {path}")
            seen.add(path)
        if not set(NON_REGISTRATION_ADDITIONS) <= seen:
            raise ReconstructionError("pre-O8v3 status lacks one of the fourteen staged additions")
    ...
    _require_direct_child(root, head, PREREGISTRATION_COMMIT, "O8v3")
    ...
    if [(p, m, o) for p, m, o, _ in index] != [(p, m, o) for p, m, o, _ in tree]:
        raise ReconstructionError("clean O8v3 stage-zero index differs from its tree")
...
def _expected_authority_identities(
    ...
    append(
        ...
        [
            "diff", "--name-status", "--no-renames", "-z",
            OPEN_FREEZE_V2_COMMIT, PREREGISTRATION_COMMIT,
        ],
    )
    append(
        "raw_audit",
    ...
    if len(commands) != 46:
        raise ReconstructionError("internal authority command plan is not 46 rows")
    return [
        _command_identity(
            attempt_index=None,
            label="authority",
