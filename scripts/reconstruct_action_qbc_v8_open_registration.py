...
PREREGISTRATION_TAG: Final = "prereg-action-qbc-v8-open-bounded-remote-verification-v5"
PREREGISTRATION_COMMIT: Final = "09f9caea346866a1acf35c20e0c9d937096b5ce3"
...
PREREGISTRATION_DOCUMENT_BLOB: Final = "7c0955a775af89dcfcde4796a9bbb4d470669d10"
PREREGISTRATION_DOCUMENT_SHA256: Final = (
    "cc9d787a64700332a44f543e7a949ee5522c3663b6b0eb54e418840e560cfe6d"
)
PREREGISTRATION_DOCUMENT_BYTE_COUNT: Final = 25_872
...
OPEN_FREEZE_TAG: Final = "action-qbc-v8-open-diagnostic-freeze-v2"
RESULT_TAG: Final = "action-qbc-v8-open-diagnostic-result-v2"
...
def _resolve_phase(root: Path) -> tuple[str, str, list[tuple[str, str, str, int | None]]]:
    ...
    if (
        ...
        or p8_v5_entry.get("git_blob_sha1") != PREREGISTRATION_DOCUMENT_BLOB
        or hashlib.sha256(p8_v5_raw).hexdigest() != PREREGISTRATION_DOCUMENT_SHA256
        or len(p8_v5_raw) != PREREGISTRATION_DOCUMENT_BYTE_COUNT
    ):
    ...
    p8_v5_tree = _tree_entries(root, PREREGISTRATION_COMMIT)
    p8_v5_document_entry = next(
        (entry for entry in p8_v5_tree if entry[0] == PREREGISTRATION_DOCUMENT),
        None,
    )
    ...
    if head == PREREGISTRATION_COMMIT:
        if _git(root, "diff", "--name-status", "--no-renames", "-z") != b"":
            raise ReconstructionError("pre-O8v2 worktree has filtered changes to staged/tracked bytes")
        cached = _git(
        ...
        if cached not in {
            ...
            raise ReconstructionError("pre-O8v2 index is not the exact fourteen- or fifteen-addition allowlist")
        ...
        for record in status.split(b"\0"):
            ...
            if len(record) < 4 or record[2:3] != b" ":
                raise ReconstructionError("malformed pre-O8v2 Git status")
            code = record[:2]
            path = record[3:].decode("utf-8")
            if path not in allowed or code not in {b"A ", b"??"}:
                raise ReconstructionError(f"non-allowlisted pre-O8v2 state: {code!r} {path}")
            seen.add(path)
        if not set(NON_REGISTRATION_ADDITIONS) <= seen:
            raise ReconstructionError("pre-O8v2 status lacks one of the fourteen staged additions")
    ...
    _require_direct_child(root, head, PREREGISTRATION_COMMIT, "O8v2")
    ...
    if [(p, m, o) for p, m, o, _ in index] != [(p, m, o) for p, m, o, _ in tree]:
        raise ReconstructionError("clean O8v2 stage-zero index differs from its tree")
...
def _expected_authority_identities(
    ...
    append(
        ...
        [
            "diff", "--name-status", "--no-renames", "-z",
            OPEN_FREEZE_V1_COMMIT, PREREGISTRATION_COMMIT,
        ],
    )
    append(
        "raw_audit",
    ...
    if len(commands) != 38:
        raise ReconstructionError("internal authority command plan is not 38 rows")
    return [
        _command_identity(
            attempt_index=None,
            label="authority",
