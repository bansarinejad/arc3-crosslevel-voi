...
_OPEN_FREEZE_TAG = "action-qbc-v8-open-diagnostic-freeze-v2"
...
_PREREGISTRATION_TAG = "prereg-action-qbc-v8-open-bounded-remote-verification-v5"
_PREREGISTRATION_COMMIT = "09f9caea346866a1acf35c20e0c9d937096b5ce3"
...
_PREREGISTRATION_DOCUMENT_BLOB = "7c0955a775af89dcfcde4796a9bbb4d470669d10"
_PREREGISTRATION_DOCUMENT_SHA256 = (
    "cc9d787a64700332a44f543e7a949ee5522c3663b6b0eb54e418840e560cfe6d"
)
_PREREGISTRATION_DOCUMENT_BYTE_COUNT = 25_872
...
_AUTHORITY_ROOT = Path("/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open/authority")
_EXECUTION_ROOT = Path("/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open")
...
def _expected_arm_argv() -> list[str]:
    return [
        ...
        "arc3-crosslevel-voi-action-qbc-v8-remote-verification-claim.json",
        "--windows-verifier-start-claim",
        "/mnt/d/kaggle competitions/"
        "arc3-crosslevel-voi-action-qbc-v8-remote-verifier-start-claim.json",
        ...
        "/mnt/d/kaggle competitions/"
        "arc3-crosslevel-voi-action-qbc-v8-remote-verification-supervisor.json",
        "--arm-receipt",
        _ARM,
    ]
...
def _repository_and_registration(root: Path) -> tuple[str, dict[str, Any], bytes]:
    ...
    if (
        ...
        or not _has_exact_parent(root, _PREREGISTRATION_COMMIT, _O8V1_COMMIT)
        or _git(
            root,
            "diff",
            "--name-status",
...
def _process(
    ...
    *,
    ...
    payload_exists = Path(output).exists() or Path(output).is_symlink()
    payload_raw: bytes | None = None
    if payload_exists:
        try:
            payload_raw = _plain(Path(output), f"process {label} payload")
        except _FinalizationError:
            payload_raw = None
    start_valid = bool(
