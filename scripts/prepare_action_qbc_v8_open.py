...
_P8V5_DOCUMENT_GIT_BLOB_SHA1: Final = (
...
_P8V6_DOCUMENT_GIT_BLOB_SHA1: Final = (
    "5e870ed0bbbff6fcb4352f6e914d870254773f68"
)
...
_EXECUTION_ROOT: Final = "/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open-v4"
...
_WINDOWS_CLAIM: Final = "/mnt/d/kaggle competitions/arc3-crosslevel-voi-action-qbc-v8-remote-verification-claim-v4.json"
_WINDOWS_START_CLAIM: Final = "/mnt/d/kaggle competitions/arc3-crosslevel-voi-action-qbc-v8-remote-verifier-start-claim-v4.json"
_WINDOWS_REMOTE_RECEIPT: Final = "/mnt/d/kaggle competitions/arc3-crosslevel-voi-action-qbc-v8-remote-verification-v4.json"
_WINDOWS_SUPERVISOR_RECEIPT: Final = "/mnt/d/kaggle competitions/arc3-crosslevel-voi-action-qbc-v8-remote-verification-supervisor-v4.json"
...
_EVIDENCE_PATHS_BY_ROLE: Final = {
    "preparation_receipt": Path(_PREPARATION_RECEIPT),
    "remote_claim": Path(_EXECUTION_ROOT) / "remote-verification-claim.json",
    "remote_verifier_claim": Path(_EXECUTION_ROOT) / "remote-verifier-start-claim.json",
    "remote_receipt": Path(_EXECUTION_ROOT) / "remote-verification.json",
    "remote_supervisor_receipt": Path(_EXECUTION_ROOT) / "remote-verification-supervisor.json",
    "arm_receipt": Path(_ARM_RECEIPT),
    "preparation_verification_receipt": Path(_PREPARATION_VERIFICATION_RECEIPT),
    "lifecycle_driver_claim": Path(_EXECUTION_ROOT) / "lifecycle-driver-claim.json",
    "lifecycle_ledger": Path(_EXECUTION_ROOT) / "lifecycle-ledger.json",
    "process_a_start_claim": Path(_EXECUTION_ROOT) / "process-a-start-claim.json",
    "process_b_start_claim": Path(_EXECUTION_ROOT) / "process-b-start-claim.json",
    "process_a_validator_claim": Path(_EXECUTION_ROOT) / "process-a-validator-claim.json",
    "process_b_validator_claim": Path(_EXECUTION_ROOT) / "process-b-validator-claim.json",
    "process_a_validation_receipt": Path(_EXECUTION_ROOT) / "process-a-validation.json",
    "process_b_validation_receipt": Path(_EXECUTION_ROOT) / "process-b-validation.json",
    "process_a_payload": Path(_EXECUTION_ROOT) / "processes/process-a-output/open/action_qbc_v8_open_diagnostic.json",
    "process_b_payload": Path(_EXECUTION_ROOT) / "processes/process-b-output/open/action_qbc_v8_open_diagnostic.json",
    "normal_finalization_bundle": Path(_EXECUTION_ROOT) / "finalization-bundle.json",
    "emergency_result_bundle": Path(_EXECUTION_ROOT) / "emergency-result-bundle.json",
    "result_git_owner_claim": Path(_EXECUTION_ROOT) / "result-git-owner.json",
}
...
def _validate_git_repository_policy(
    ...
    *,
    ...
    if _parse_local_git_config(config_raw) != _LOCAL_GIT_CONFIG:
        _fail(f"local Git config differs from the closed P8v7 mapping: {root}")
...
def _derive_open_freeze(
    ...
    if _git(ledger, authority, "cat-file", "-t", f"refs/tags/{_P8V5_TAG}") != b"commit\n":
        _fail("P8v5 tag is absent or annotated")
    if _git(ledger, authority, "rev-parse", f"refs/tags/{_P8V5_TAG}") != f"{_P8V5_COMMIT}\n".encode("ascii"):
    ...
    if p8v5_delta != _expected_forward_reset_name_status(
        _ALL_ADDITIONS, _P8V5_DOCUMENT
    ):
        _fail("O8v1..P8v5 is not the exact forward-reset delta")
    if _git(ledger, authority, "cat-file", "-t", f"refs/tags/{_O8V2_TAG}") != b"commit\n":
        _fail("O8v2 tag is absent or annotated")
    if _git(ledger, authority, "rev-parse", f"refs/tags/{_O8V2_TAG}") != f"{_O8V2_COMMIT}\n".encode("ascii"):
        _fail("O8v2 tag differs from the immutable second open freeze")
    _require_direct_child(
        ledger,
        authority,
        _O8V2_COMMIT,
    ...
    if o8v2_delta != _expected_name_status(_ALL_ADDITIONS):
        _fail("P8v5..O8v2 is not the exact fifteen-path historical open-freeze delta")
    if _git(ledger, authority, "cat-file", "-t", f"refs/tags/{_P8V6_TAG}") != b"commit\n":
        _fail("P8v6 tag is absent or annotated")
    if _git(ledger, authority, "rev-parse", f"refs/tags/{_P8V6_TAG}") != f"{_P8V6_COMMIT}\n".encode("ascii"):
        _fail("P8v6 tag differs from the frozen runner-manifest-key recovery commit")
    _require_direct_child(
        ledger,
        authority,
        _P8V6_COMMIT,
        _O8V2_COMMIT,
        "P8v6 is not a direct child of O8v2",
    )
    p8v6_delta = _git(
        ledger,
        authority,
    ...
    if p8v6_delta != _expected_forward_reset_name_status(
        _ALL_ADDITIONS, _P8V6_DOCUMENT
    ):
        _fail("O8v2..P8v6 is not the exact forward-reset delta")

    if _git(ledger, authority, "cat-file", "-t", f"refs/tags/{_O8V3_TAG}") != b"commit\n":
        _fail("O8v3 tag is absent or annotated")
    if _git(ledger, authority, "rev-parse", f"refs/tags/{_O8V3_TAG}") != f"{_O8V3_COMMIT}\n".encode("ascii"):
        _fail("O8v3 tag differs from the immutable third open freeze")
    _require_direct_child(
        ledger,
        authority,
        _O8V3_COMMIT,
        _P8V6_COMMIT,
        "O8v3 is not a direct child of P8v6",
    )
    o8v3_delta = _git(
        ledger,
        authority,
    ...
    if o8v3_delta != _expected_name_status(_ALL_ADDITIONS):
    ...
    if _git(ledger, authority, "cat-file", "-t", f"refs/tags/{_PREREGISTRATION_TAG}") != b"commit\n":
        _fail("P8v7 tag is absent or annotated")
    if _git(ledger, authority, "rev-parse", f"refs/tags/{_PREREGISTRATION_TAG}") != f"{_PREREGISTRATION_COMMIT}\n".encode("ascii"):
        _fail("P8v7 tag differs from the frozen consumed-lifecycle recovery commit")
    _require_direct_child(
        ledger,
        authority,
    ...
    if p8v7_delta != _expected_forward_reset_name_status(
    ...
    if document_tree != expected_tree:
        _fail("P8v7 recovery document Git object identity is invalid")
    document_bytes = _git(
        ledger,
        authority,
        "cat-file",
    ...
    if (
        ...
        _fail("P8v7 recovery document raw bytes are invalid")
    _require_direct_child(
        ledger,
        authority,
        commit,
...
def _validate_execution_contract(execution: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        _fail("registration preparation-verification argv differs from P8v7")
    ...
    if execution.get("preparation_command_environment") != _PREPARATION_COMMAND_ENVIRONMENT:
        _fail("registration preparation command environment differs from P8v7")
    if execution.get("preparation_command_policy") != _PREPARATION_COMMAND_POLICY:
        _fail("registration preparation command policy differs from P8v7")
...
def _publish_bytes_exclusive(
    path: Path, data: bytes, name: str, *, role: str
) -> None:
    ...
    try:
        ...
        if (
            ...
            or stat.S_IMODE(staged_metadata.st_mode) != expected_mode
        ...
        if (
            ...
            or stat.S_IMODE(published_metadata.st_mode) != expected_mode
        ...
        if (
            ...
            or stat.S_IMODE(final_metadata.st_mode) != expected_mode
...
def _artifact_state(
    ...
    *,
    ...
    expected_mode = _expected_evidence_mode(path, role) if role is not None else None
    try:
        metadata = path.lstat()
    except FileNotFoundError:
    ...
    if verification_state.raw is None:
        ...
        try:
            ...
            if (
                ...
                _fail("preparation verification receipt does not match live P8v7 state")
...
def _source_artifact(path: Path, name: str) -> _SourceArtifact:
    state = _artifact_state(path, name, role=None, maximum=_REMOTE_ARTIFACT_LIMIT)
    ...
    for name in ("claim", "start", "remote", "supervisor"):
        ...
        if raw is not None:
            _publish_bytes_exclusive(
                ...
                role={
                    "claim": "remote_claim",
                    "start": "remote_verifier_claim",
                    "remote": "remote_receipt",
                    "supervisor": "remote_supervisor_receipt",
                }[name],
            )
    armed = (
