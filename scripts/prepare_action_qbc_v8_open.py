...
def _derive_open_freeze(
    ...
    if v4_delta != b"A\0" + _PREREGISTRATION_DOCUMENT.encode("utf-8") + b"\0":
    ...
    if document_tree != expected_tree:
        _fail("P8v4 correction document Git object identity is invalid")
    document_bytes = _git(
        ledger,
        authority,
        "cat-file",
    ...
    if (
        ...
        _fail("P8v4 correction document raw bytes are invalid")
    _require_direct_child(
        ledger,
        authority,
        commit,
        _PREREGISTRATION_COMMIT,
        "O8 is not a direct child of P8v4",
    )
    delta = _git(
        ledger,
        authority,
    ...
    if delta != _expected_name_status(_ALL_ADDITIONS):
        _fail("P8v4..O8 is not the exact fifteen-path open-freeze delta")
    return commit
