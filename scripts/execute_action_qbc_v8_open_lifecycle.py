...
_PREREGISTRATION_TAG: Final = "prereg-action-qbc-v8-open-bounded-remote-verification-v6"
_PREREGISTRATION_COMMIT: Final = "61cebe90a2f4f7c78ec45119de53a482ed13a655"
_PREREGISTRATION_TREE: Final = "65695876c44eeb8cac5437149384071f88ff6018"
...
_PREREGISTRATION_DOCUMENT_BYTE_COUNT: Final = 32_370
...
_RESULT_TAG: Final = "action-qbc-v8-open-diagnostic-result-v3"
...
def _derive_o8(authority: Path, environment: Mapping[str, str]) -> str:
    ...
    ) != f"{_PREREGISTRATION_COMMIT} {_O8V2_COMMIT}\n".encode("ascii"):
    ...
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _O8V2_COMMIT, _PREREGISTRATION_COMMIT,
    ...
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _P8V5_COMMIT, _PREREGISTRATION_COMMIT,
    ...
    _hex(commit, 40, "O8v3 commit")
    raw = _git(authority, environment, "cat-file", "-p", commit)
    if _git_oid("commit", raw) != commit:
        _fail("O8v3 commit bytes do not reproduce its object ID")
    header, separator, _message = raw.partition(b"\n\n")
    if not separator:
        _fail("O8v3 commit object has no header/message separator")
    parents = [
        _hex(line[7:].decode("ascii", "strict"), 40, "O8v3 parent")
        for line in header.splitlines()
        if line.startswith(b"parent ")
    ]
    if parents != [_PREREGISTRATION_COMMIT]:
        _fail("O8v3 is not an exact one-parent direct child of P8v6")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
    ...
    if _git(authority, environment, "rev-parse", "HEAD").decode("ascii", "strict").strip() != commit:
        _fail("authority HEAD differs from O8v3")
    return commit
...
def _load_registration(
    ...
    if (
        ...
        _fail("registration P8v6 identity is invalid")
...
def _preparation_expected_authority_identities(
    ...
    *,
    ...
    for prior_tag, prior_commit, parent_commit in (
        ...
        (_PREREGISTRATION_TAG, _PREREGISTRATION_COMMIT, _O8V2_COMMIT),
    ):
    ...
    if len(commands) != 46:
        _fail("internal preparation authority command plan is not exactly 46 rows")
    return [
        _preparation_command_identity(
            attempt_index=None,
            label="authority",
