...
_RESULT_TAG: Final = "action-qbc-v8-open-diagnostic-result-v2"
...
def _derive_o8(authority: Path, environment: Mapping[str, str]) -> str:
    ...
    ) != f"{_PREREGISTRATION_COMMIT} {_O8V1_COMMIT}\n".encode("ascii"):
        _fail("P8v5 is not the exact direct child of O8v1")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _O8V1_COMMIT, _PREREGISTRATION_COMMIT,
    ...
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _P8V4_COMMIT, _PREREGISTRATION_COMMIT,
    ...
    _hex(commit, 40, "O8v2 commit")
    raw = _git(authority, environment, "cat-file", "-p", commit)
    if _git_oid("commit", raw) != commit:
        _fail("O8v2 commit bytes do not reproduce its object ID")
    header, separator, _message = raw.partition(b"\n\n")
    if not separator:
        _fail("O8v2 commit object has no header/message separator")
    parents = [
        _hex(line[7:].decode("ascii", "strict"), 40, "O8v2 parent")
        for line in header.splitlines()
        if line.startswith(b"parent ")
    ]
    if parents != [_PREREGISTRATION_COMMIT]:
        _fail("O8v2 is not an exact one-parent direct child of P8v5")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
    ...
    if _git(authority, environment, "rev-parse", "HEAD").decode("ascii", "strict").strip() != commit:
        _fail("authority HEAD differs from O8v2")
    return commit
...
def _load_registration(
    ...
    if (
        ...
        _fail("registration P8v5 identity is invalid")
...
def _preparation_expected_authority_identities(
    ...
    *,
    ...
    for prior_tag, prior_commit, parent_commit in (
        ...
        (_PREREGISTRATION_TAG, _PREREGISTRATION_COMMIT, _O8V1_COMMIT),
    ):
    ...
    if len(commands) != 38:
        _fail("internal preparation authority command plan is not exactly 38 rows")
    return [
        _preparation_command_identity(
            attempt_index=None,
            label="authority",
