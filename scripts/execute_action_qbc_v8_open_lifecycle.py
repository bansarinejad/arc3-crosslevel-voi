...
_OPEN_FREEZE_TAG: Final = "action-qbc-v8-open-diagnostic-freeze-v4"
...
_P8V5_TAG: Final = "prereg-action-qbc-v8-open-bounded-remote-verification-v5"
_P8V5_COMMIT: Final = "09f9caea346866a1acf35c20e0c9d937096b5ce3"
_P8V5_TREE: Final = "47a978cdd887fd6dc1cb5e80e36aa3e0a5a29253"
_O8V2_TAG: Final = "action-qbc-v8-open-diagnostic-freeze-v2"
_O8V2_COMMIT: Final = "8da637a47de0c88f917f222e52e54b342d729be9"
_O8V2_TREE: Final = "247eba59e1e2ac9b0611c0e361de945dae0f2dc8"
_P8V6_TAG: Final = "prereg-action-qbc-v8-open-bounded-remote-verification-v6"
_P8V6_COMMIT: Final = "61cebe90a2f4f7c78ec45119de53a482ed13a655"
_P8V6_TREE: Final = "65695876c44eeb8cac5437149384071f88ff6018"
_O8V3_TAG: Final = "action-qbc-v8-open-diagnostic-freeze-v3"
_O8V3_COMMIT: Final = "5725395a850627fae10e8bb8b27083ccf63b6ec7"
_O8V3_TREE: Final = "7d38de8f5cec16cab92c9d3b757a218e8e490272"
_PREREGISTRATION_TAG: Final = "prereg-action-qbc-v8-open-bounded-remote-verification-v7"
_PREREGISTRATION_COMMIT: Final = "15059c482d9e463f01cb31fdfd33c96d1f60db0a"
_PREREGISTRATION_TREE: Final = "96469ca9ee018cd32f99955df1ded57af12a8abc"
_P8V5_DOCUMENT: Final = (
    "docs/experiment_amendment_2026-08-18_action_qbc_v8_open_bounded_remote_verification_v5_public_visibility_recovery.md"
)
_P8V5_DOCUMENT_BLOB: Final = "7c0955a775af89dcfcde4796a9bbb4d470669d10"
_P8V5_DOCUMENT_SHA256: Final = (
    "cc9d787a64700332a44f543e7a949ee5522c3663b6b0eb54e418840e560cfe6d"
)
_P8V5_DOCUMENT_BYTE_COUNT: Final = 25_872
_P8V6_DOCUMENT: Final = (
    "docs/experiment_amendment_2026-08-18_action_qbc_v8_open_bounded_remote_verification_v6_runner_manifest_key_recovery.md"
)
_P8V6_DOCUMENT_BLOB: Final = "5e870ed0bbbff6fcb4352f6e914d870254773f68"
_P8V6_DOCUMENT_SHA256: Final = (
    "0ba4cc55ca2b31433bc458972ffc32d87f84b610673fa22ed2b4dd4a8bfc1a41"
)
_P8V6_DOCUMENT_BYTE_COUNT: Final = 32_370
...
_PREREGISTRATION_DOCUMENT_BLOB: Final = "c0cda2417bd98a42b76e8e1bbdee4cec01dd68f9"
_PREREGISTRATION_DOCUMENT_SHA256: Final = (
    "f729904367dd7a2664ecd3fdfe4893841326668fbe892b3b733926ad7840745d"
)
_PREREGISTRATION_DOCUMENT_BYTE_COUNT: Final = 37_552
...
_P8V4_DOCUMENT_BLOB: Final = "29c991b7e23209f2c38d5e9a11a15bca51753d8e"
...
_EXECUTION_ROOT: Final = Path("/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open-v4")
...
_WINDOWS_CLAIM: Final = Path("/mnt/d/kaggle competitions/arc3-crosslevel-voi-action-qbc-v8-remote-verification-claim-v4.json")
...
_EVIDENCE_PATHS_BY_ROLE: Final = {
    "preparation_receipt": _PREPARATION,
    "remote_claim": _REMOTE_CLAIM,
    "remote_verifier_claim": _REMOTE_VERIFIER,
    ...
    "lifecycle_driver_claim": _DRIVER,
    "lifecycle_ledger": _LEDGER,
    "process_a_start_claim": _A_START,
    "process_b_start_claim": _B_START,
    "process_a_validator_claim": _A_VALIDATOR,
    "process_b_validator_claim": _B_VALIDATOR,
    "process_a_validation_receipt": _A_VALIDATION,
    "process_b_validation_receipt": _B_VALIDATION,
    "process_a_payload": _A_OUTPUT,
    "process_b_payload": _B_OUTPUT,
    "normal_finalization_bundle": _FINAL_BUNDLE,
    "emergency_result_bundle": _EMERGENCY_BUNDLE,
    "result_git_owner_claim": _OWNER_CLAIM,
}
...
def _bundle_evidence_role(path: Path) -> str:
    if path == _FINAL_BUNDLE:
        return "normal_finalization_bundle"
    if path == _EMERGENCY_BUNDLE:
        return "emergency_result_bundle"
    _fail("selected bundle path differs from registration")
...
def _evidence_state(
    ...
    *,
    ...
    expected_mode = _expected_evidence_mode(path, role)
...
def _optional_raw(path: Path, *, role: str) -> bytes | None:
    return _evidence_state(path, str(path), role=role).raw
...
def _raw_sha_or_none(path: Path, *, role: str) -> str | None:
    raw = _optional_raw(path, role=role)
...
def _exclusive_json(path: Path, value: Mapping[str, Any], *, role: str) -> bytes:
    raw = canonical_json_bytes(value)
    _exclusive_bytes(path, raw, mode=_expected_evidence_mode(path, role))
    reopened = _evidence_state(path, str(path), role=role)
...
def _expected_name_status(paths: Sequence[str], status: str = "A") -> bytes:
    return b"".join(
...
def _derive_o8(authority: Path, environment: Mapping[str, str]) -> str:
    ...
    for commit_id, path, blob, digest, count, label in (
        ...
        (_P8V4_COMMIT, _P8V4_DOCUMENT, _P8V4_DOCUMENT_BLOB, _P8V4_DOCUMENT_SHA256, _P8V4_DOCUMENT_BYTE_COUNT, "P8v4"),
    ):
    ...
    if (
        _git(authority, environment, "cat-file", "-t", o8v1_ref) != b"commit\n"
        or _git(authority, environment, "rev-parse", o8v1_ref)
        != f"{_O8V1_COMMIT}\n".encode("ascii")
    ):
        _fail("O8v1 lightweight tag identity is invalid")
    if _git(
        authority, environment, "rev-list", "--parents", "-n", "1", _O8V1_COMMIT
    ) != f"{_O8V1_COMMIT} {_P8V4_COMMIT}\n".encode("ascii"):
        _fail("O8v1 is not the exact direct child of P8v4")
    if _git(
        authority, environment, "rev-parse", f"{_O8V1_COMMIT}^{{tree}}"
    ) != f"{_O8V1_TREE}\n".encode("ascii"):
        _fail("O8v1 tree identity is invalid")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _P8V4_COMMIT, _O8V1_COMMIT,
    ) != _expected_name_status(_O8_ADDITIONS):
        _fail("P8v4..O8v1 is not the exact fifteen-path historical freeze delta")

    p8v5_ref = f"refs/tags/{_P8V5_TAG}"
    if (
        _git(authority, environment, "cat-file", "-t", p8v5_ref)
        != b"commit\n"
        or _git(authority, environment, "rev-parse", p8v5_ref)
        != f"{_P8V5_COMMIT}\n".encode("ascii")
    ):
        _fail("P8v5 lightweight tag identity is invalid")
    if _git(
        authority,
        environment,
        "rev-list",
        ...
        "1",
        _P8V5_COMMIT,
    ) != f"{_P8V5_COMMIT} {_O8V1_COMMIT}\n".encode("ascii"):
        _fail("P8v5 is not the exact direct child of O8v1")
    if _git(
        authority, environment, "rev-parse", f"{_P8V5_COMMIT}^{{tree}}"
    ) != f"{_P8V5_TREE}\n".encode("ascii"):
        _fail("P8v5 tree identity is invalid")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _O8V1_COMMIT, _P8V5_COMMIT,
    ) != _expected_forward_reset_name_status(_O8_ADDITIONS, _P8V5_DOCUMENT):
        _fail("O8v1..P8v5 is not the exact forward-reset delta")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _P8V4_COMMIT, _P8V5_COMMIT,
    ) != _expected_name_status((_P8V5_DOCUMENT,)):
        _fail("P8v5 tree is not the P8v4 tree plus the recovery document")
    p8v5_document_raw = _git(
        authority,
        environment,
        "cat-file",
        "blob",
        f"{_P8V5_COMMIT}:{_P8V5_DOCUMENT}",
    )
    if (
        _git_oid("blob", p8v5_document_raw) != _P8V5_DOCUMENT_BLOB
    ...
    if (
        _git(authority, environment, "cat-file", "-t", o8v2_ref) != b"commit\n"
        or _git(authority, environment, "rev-parse", o8v2_ref)
        != f"{_O8V2_COMMIT}\n".encode("ascii")
    ):
        _fail("O8v2 lightweight tag identity is invalid")
    if _git(
        authority, environment, "rev-list", "--parents", "-n", "1", _O8V2_COMMIT
    ) != f"{_O8V2_COMMIT} {_P8V5_COMMIT}\n".encode("ascii"):
        _fail("O8v2 is not the exact direct child of P8v5")
    if _git(
        authority, environment, "rev-parse", f"{_O8V2_COMMIT}^{{tree}}"
    ) != f"{_O8V2_TREE}\n".encode("ascii"):
        _fail("O8v2 tree identity is invalid")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _P8V5_COMMIT, _O8V2_COMMIT,
    ) != _expected_name_status(_O8_ADDITIONS):
        _fail("P8v5..O8v2 is not the exact fifteen-path historical freeze delta")

    p8v6_ref = f"refs/tags/{_P8V6_TAG}"
    if (
        _git(authority, environment, "cat-file", "-t", p8v6_ref)
        != b"commit\n"
        or _git(authority, environment, "rev-parse", p8v6_ref)
        != f"{_P8V6_COMMIT}\n".encode("ascii")
    ):
        _fail("P8v6 lightweight tag identity is invalid")
    if _git(
        authority,
        environment,
        "rev-list",
        ...
        "1",
        _P8V6_COMMIT,
    ) != f"{_P8V6_COMMIT} {_O8V2_COMMIT}\n".encode("ascii"):
        _fail("P8v6 is not the exact direct child of O8v2")
    if _git(
        authority, environment, "rev-parse", f"{_P8V6_COMMIT}^{{tree}}"
    ) != f"{_P8V6_TREE}\n".encode("ascii"):
        _fail("P8v6 tree identity is invalid")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _O8V2_COMMIT, _P8V6_COMMIT,
    ) != _expected_forward_reset_name_status(_O8_ADDITIONS, _P8V6_DOCUMENT):
        _fail("O8v2..P8v6 is not the exact forward-reset delta")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _P8V5_COMMIT, _P8V6_COMMIT,
    ) != _expected_name_status((_P8V6_DOCUMENT,)):
        _fail("P8v6 tree is not the P8v5 tree plus the recovery document")
    p8v6_document_raw = _git(
        authority,
        environment,
        "cat-file",
        "blob",
        f"{_P8V6_COMMIT}:{_P8V6_DOCUMENT}",
    )
    if (
        _git_oid("blob", p8v6_document_raw) != _P8V6_DOCUMENT_BLOB
    ...
    if (
        _git(authority, environment, "cat-file", "-t", o8v3_ref) != b"commit\n"
        or _git(authority, environment, "rev-parse", o8v3_ref)
        != f"{_O8V3_COMMIT}\n".encode("ascii")
    ):
        _fail("O8v3 tag is absent or annotated")
    if _git(
        authority, environment, "rev-list", "--parents", "-n", "1", _O8V3_COMMIT
    ) != f"{_O8V3_COMMIT} {_P8V6_COMMIT}\n".encode("ascii"):
        _fail("O8v3 is not the exact direct child of P8v6")
    if _git(
        authority, environment, "rev-parse", f"{_O8V3_COMMIT}^{{tree}}"
    ) != f"{_O8V3_TREE}\n".encode("ascii"):
        _fail("O8v3 tree identity is invalid")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _P8V6_COMMIT, _O8V3_COMMIT,
    ) != _expected_name_status(_O8_ADDITIONS):
        _fail("P8v6..O8v3 is not the exact fifteen-path open-freeze delta")

    preregistration_ref = f"refs/tags/{_PREREGISTRATION_TAG}"
    if (
        _git(authority, environment, "cat-file", "-t", preregistration_ref)
        != b"commit\n"
        or _git(authority, environment, "rev-parse", preregistration_ref)
        != f"{_PREREGISTRATION_COMMIT}\n".encode("ascii")
    ):
        _fail("P8v7 lightweight tag identity is invalid")
    if _git(
        authority,
        environment,
        "rev-list",
    ...
    ) != f"{_PREREGISTRATION_COMMIT} {_O8V3_COMMIT}\n".encode("ascii"):
        _fail("P8v7 is not the exact direct child of O8v3")
    if _git(
        authority, environment, "rev-parse", f"{_PREREGISTRATION_COMMIT}^{{tree}}"
    ) != f"{_PREREGISTRATION_TREE}\n".encode("ascii"):
        _fail("P8v7 tree identity is invalid")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _O8V3_COMMIT, _PREREGISTRATION_COMMIT,
    ) != _expected_forward_reset_name_status(_O8_ADDITIONS, _PREREGISTRATION_DOCUMENT):
        _fail("O8v3..P8v7 is not the exact consumed-lifecycle forward reset")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _P8V6_COMMIT, _PREREGISTRATION_COMMIT,
    ) != _expected_name_status((_PREREGISTRATION_DOCUMENT,)):
        _fail("P8v7 tree is not the P8v6 tree plus the recovery document")
    document_raw = _git(
        authority,
        environment,
        "cat-file",
    ...
    if (
        _git_oid("blob", document_raw) != _PREREGISTRATION_DOCUMENT_BLOB
    ...
    if _git(authority, environment, "cat-file", "-t", tag_ref) != b"commit\n":
        _fail("O8v4 tag is absent or annotated")
    commit = _git(authority, environment, "rev-parse", tag_ref).decode("ascii", "strict").strip()
    _hex(commit, 40, "O8v4 commit")
    raw = _git(authority, environment, "cat-file", "-p", commit)
    if _git_oid("commit", raw) != commit:
        _fail("O8v4 commit bytes do not reproduce its object ID")
    header, separator, _message = raw.partition(b"\n\n")
    if not separator:
        _fail("O8v4 commit object has no header/message separator")
    parents = [
        _hex(line[7:].decode("ascii", "strict"), 40, "O8v4 parent")
        for line in header.splitlines()
        if line.startswith(b"parent ")
    ]
    if parents != [_PREREGISTRATION_COMMIT]:
        _fail("O8v4 is not an exact one-parent direct child of P8v7")
    if _git(
        authority, environment, "diff", "--name-status", "--no-renames", "-z",
        _PREREGISTRATION_COMMIT, commit,
    ) != _expected_name_status(_O8_ADDITIONS):
        _fail("P8v7..O8v4 is not the exact fifteen-path open-freeze delta")
    if _git(authority, environment, "rev-parse", "HEAD").decode("ascii", "strict").strip() != commit:
        _fail("authority HEAD differs from O8v4")
    return commit
...
def _load_registration(
    ...
    if (
        ...
        or preregistration.get("commit_sha") != _PREREGISTRATION_COMMIT
        ...
        _fail("registration P8v7 identity is invalid")
...
def _require_registered_execution(
    ...
    *,
    ...
    if set(execution) != _EXECUTION_KEYS:
        _fail("registration execution contract does not have exactly the P8v7 70-key schema")
    if execution.get("windows_repository_contract") != _WINDOWS_REPOSITORY_CONTRACT:
        _fail("registered Windows repository contract differs from P8v7")
    fixed_paths = {
    ...
    if len(publisher_argv) != 23 or len(publisher_argv[4:]) != 19:
        _fail("registered result-publisher wrapper/inner argv partition is invalid")
    ...
    if publish_argv is not None and list(publish_argv) != publisher_argv[4:]:
...
def _acquire_driver_claim(
    *,
    ...
    raw = _exclusive_json(_DRIVER, claim, role="lifecycle_driver_claim")
    return claim, raw


def _artifact_hash(path: Path, *, role: str) -> str | None:
    raw = _optional_raw(path, role=role)
    return _sha256(raw) if raw is not None else None
...
def _update_process_artifacts(
    ...
    *,
    ...
    prefix = "process_a" if record.get("label") == "A" else "process_b"
    record["start_claim_sha256"] = _artifact_hash(
        start, role=f"{prefix}_start_claim"
    )
    record["validator_claim_sha256"] = _artifact_hash(
        validator, role=f"{prefix}_validator_claim"
    )
    record["validation_receipt_sha256"] = _artifact_hash(
        validation, role=f"{prefix}_validation_receipt"
    )
    record["output_sha256"] = _artifact_hash(output, role=f"{prefix}_payload")
...
def _remote_and_arm_stage(
    *,
    ...
    preparation_state = _evidence_state(
        _PREPARATION, "preparation receipt", role="preparation_receipt"
    )
    if (
    ...
    claim, claim_raw = _valid_receipt(
        _REMOTE_CLAIM, role="remote_claim", keys=_REMOTE_CLAIM_KEYS,
        schema="action-qbc-v8-remote-tag-verification-claim-v1",
        commit=commit, registration_sha=registration_sha,
    )
    verifier, verifier_raw = _valid_receipt(
        _REMOTE_VERIFIER, role="remote_verifier_claim", keys=_REMOTE_VERIFIER_KEYS,
        schema="action-qbc-v8-remote-tag-verifier-start-claim-v1",
        commit=commit, registration_sha=registration_sha,
    )
    receipt, receipt_raw = _valid_receipt(
        _REMOTE_RECEIPT, role="remote_receipt", keys=_REMOTE_RECEIPT_KEYS,
    ...
    arm, arm_raw = _valid_receipt(
        _ARM, role="arm_receipt", keys=_ARM_KEYS, schema=_ARM_SCHEMA, commit=commit,
        registration_sha=registration_sha,
    )
    if not (
...
def _validation_is_valid(
    ...
    *,
    ...
    prefix = "process_a" if label == "A" else "process_b"
    raw = _optional_raw(path, role=f"{prefix}_validation_receipt")
    payload = _optional_raw(output, role=f"{prefix}_payload")
    start_path = _A_START if label == "A" else _B_START
    validator_path = _A_VALIDATOR if label == "A" else _B_VALIDATOR
    start_raw = _optional_raw(start_path, role=f"{prefix}_start_claim")
    validator_raw = _optional_raw(validator_path, role=f"{prefix}_validator_claim")
...
def _ledger_value(
    *,
    ...
    result = {
        ...
        "arm_receipt_sha256": _artifact_hash(_ARM, role="arm_receipt"),
...
def _validate_embedded_remote_receipt_common(
    ...
    *,
    ...
    if total < sum(durations) + retry_delay * max(0, len(attempts) - 1):
        _fail("embedded remote duration violates the exact P8v7 lower bound")
    return str(status)
...
def _validate_bundled_machine_result(
    ...
    *,
    ...
    if disposition == "scientific_result":
        ...
        payload_a = _evidence_state(
            _A_OUTPUT, "reopened process-A payload", role="process_a_payload"
        ).raw
        payload_b = _evidence_state(
            _B_OUTPUT, "reopened process-B payload", role="process_b_payload"
        ).raw
        if (
            payload_a is None
...
def _validate_bundle_bytes(
    ...
    *,
    ...
    if emergency:
        ...
        finalization_state = _evidence_state(
            _FINAL_BUNDLE,
            "finalization-bundle evidence",
            role="normal_finalization_bundle",
        )
        ledger_state = _evidence_state(
            _LEDGER, "lifecycle-ledger evidence", role="lifecycle_ledger"
        )
        preparation_state = _evidence_state(
            _PREPARATION, "preparation-receipt evidence", role="preparation_receipt"
        )
        verification_state = _evidence_state(
            _PREPARATION_VERIFICATION,
...
def _selected_bundle(
    *,
    ...
    emergency_state = _evidence_state(
        _EMERGENCY_BUNDLE,
        "emergency result bundle",
        role="emergency_result_bundle",
    )
    ...
    normal_state = _evidence_state(
        _FINAL_BUNDLE,
        "normal finalization bundle",
        role="normal_finalization_bundle",
    )
    ...
    if emergency_raw is not None:
        ...
        if normal_state.exists:
            ...
            if (
                ...
                _fail("normal/emergency coexistence is not the sole P8v7 cleanup override")
...
def _publish_emergency_bundle(
    ...
    *,
    ...
    finalization = _evidence_state(
        _FINAL_BUNDLE,
        "raw finalization-bundle evidence",
        role="normal_finalization_bundle",
    )
    ledger = _evidence_state(
        _LEDGER, "raw lifecycle-ledger evidence", role="lifecycle_ledger"
    )
    preparation = _evidence_state(
        _PREPARATION, "raw preparation-receipt evidence", role="preparation_receipt"
    )
    preparation_verification = _evidence_state(
        _PREPARATION_VERIFICATION,
        "raw preparation-verification evidence",
        role="preparation_verification_receipt",
    )
    ...
    raw = _exclusive_json(
        _EMERGENCY_BUNDLE, bundle, role="emergency_result_bundle"
    )
...
def _ensure_owner(
    *,
    ...
    if _OWNER_CLAIM.exists() or _OWNER_CLAIM.is_symlink():
        owner_state = _evidence_state(
            _OWNER_CLAIM,
            "result-Git owner claim",
            role="result_git_owner_claim",
        )
        observed, raw = owner_state.value, owner_state.raw
        if (
            raw is None
            or observed is None
            or set(observed) != _OWNER_KEYS
            or observed.get("schema_version") != _OWNER_SCHEMA
            or observed.get("treatment_id") != _TREATMENT_ID
        ):
            _fail("existing result-Git owner claim is invalid")
        if observed != expected:
            _fail("existing result-Git owner claim differs from the registered identity")
    else:
        raw = _exclusive_json(
            _OWNER_CLAIM, expected, role="result_git_owner_claim"
        )
    ...
    if (
        ...
        != _expected_evidence_mode(_OWNER_CLAIM, "result_git_owner_claim")
    ):
...
def _load_driver_claim(
    *,
    ...
    state = _evidence_state(
        _DRIVER,
        "lifecycle driver claim",
        role="lifecycle_driver_claim",
    )
    value, raw = state.value, state.raw
    if (
        raw is None
        or value is None
        or set(value) != _DRIVER_KEYS
        or value.get("schema_version") != _DRIVER_SCHEMA
        or value.get("treatment_id") != _TREATMENT_ID
    ):
        _fail("lifecycle driver claim is invalid")
    hashes = execution.get("argv_hashes")
    if not isinstance(hashes, Mapping):
        _fail("registration argv hashes are absent")
    if (
...
def _publication_attempt(
    ...
    *,
    ...
    if _evidence_state(
        bundle_path,
        "selected immutable bundle",
        role=_bundle_evidence_role(bundle_path),
    ).raw != bundle_raw:
    ...
    *,
    ...
    try:
        ...
        if stage is None:
            ...
            elif _optional_raw(_A_OUTPUT, role="process_a_payload") is None:
        ...
        if stage is None:
            ...
            elif _optional_raw(_B_OUTPUT, role="process_b_payload") is None:
        ...
        if stage is None:
            final_a_raw = _evidence_state(
                _A_OUTPUT,
                "validated process-A payload",
                role="process_a_payload",
            ).raw
            final_b_raw = _evidence_state(
                _B_OUTPUT,
                "validated process-B payload",
                role="process_b_payload",
            ).raw
            if final_a_raw is None or final_b_raw is None:
                _fail("validated payload disappeared before byte comparison")
            if final_a_raw != final_b_raw:
...
def _publish_lifecycle_ledger(
    *,
    ...
    try:
        raw = _exclusive_json(_LEDGER, normal, role="lifecycle_ledger")
        ...
        if not _LEDGER.exists() and not _LEDGER.is_symlink():
            ...
            try:
                raw = _exclusive_json(_LEDGER, minimal, role="lifecycle_ledger")
                return raw, False
            except Exception:
                pass
        return _optional_raw(_LEDGER, role="lifecycle_ledger"), False
...
def _finish_lifecycle(
    *,
    ...
    try:
        ...
        ledger_raw, ledger_normal = (
            _optional_raw(_LEDGER, role="lifecycle_ledger"),
            False,
        )
    ...
    if finalizer.classification == "child_cleanup_failed":
        # P8v7 makes failed outer PGID cleanup terminal.  Local emergency evidence is
