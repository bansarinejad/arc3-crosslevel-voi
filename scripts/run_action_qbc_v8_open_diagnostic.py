...
_PREREGISTRATION_TREE = "47a978cdd887fd6dc1cb5e80e36aa3e0a5a29253"
...
def _validate_recovery_lineage(root: Path, open_commit: str) -> None:
    ...
    if (
        ...
        _fail("P8v5 tree differs from its immutable registered identity")
    reset_delta = {("D", path) for path in _O8_ADDITIONS}
    reset_delta.add(("A", _PREREGISTRATION_DOCUMENT))
    _require_delta(
        root,
        _O8V1_COMMIT,
        _PREREGISTRATION_COMMIT,
    ...
    if listing != expected_listing:
        _fail("P8v5 recovery document Git identity is invalid")
    document_raw = _git(
        root,
        "cat-file",
        "blob",
    ...
    if (
        ...
        _fail("P8v5 recovery document raw identity is invalid")
...
def _validate_reconstructed_execution(
    ...
    additions = source_manifest.get("added_in_open_freeze")
...
def _expected_authority_identities(
    ...
    if len(commands) != 38:
        _fail("internal authority command plan is not 38 rows")
    return [
        _command_identity(
            attempt_index=None,
            label="authority",
