...
_EXECUTION_ROOT_TEXT = "/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open-v4"
...
def _validate_recovery_lineage(root: Path, open_commit: str) -> None:
    ...
    if (
        _git(root, "rev-parse", f"{_P8V6_COMMIT}^{{tree}}")
        != f"{_P8V6_TREE}\n".encode("ascii")
    ):
        _fail("P8v6 tree differs from its immutable historical identity")
    p8v6_reset_delta = {("D", path) for path in _O8_ADDITIONS}
    p8v6_reset_delta.add(("A", _P8V6_DOCUMENT))
    _require_delta(
        root,
        _O8V2_COMMIT,
        _P8V6_COMMIT,
    ...
    expected_listing = (
        f"100644 blob {_P8V6_DOCUMENT_BLOB}\t"
        f"{_P8V6_DOCUMENT}\0"
    ).encode()
    listing = _git(
        root,
    ...
    if (
        len(document_raw) != _P8V6_DOCUMENT_BYTE_COUNT
        or _sha256(document_raw) != _P8V6_DOCUMENT_SHA256
    ):
        _fail("P8v6 recovery document raw identity is invalid")

    _require_lightweight_tag(root, _O8V3_TAG, _O8V3_COMMIT, name="O8v3 tag")
    _require_direct_child(
        root,
        _O8V3_COMMIT,
        _P8V6_COMMIT,
    ...
    if (
        _git(root, "rev-parse", f"{_O8V3_COMMIT}^{{tree}}")
        != f"{_O8V3_TREE}\n".encode("ascii")
    ):
        _fail("O8v3 tree differs from its immutable historical identity")
    _require_delta(
        root,
        _P8V6_COMMIT,
        _O8V3_COMMIT,
    ...
    if (
        ...
        _fail("P8v7 tree differs from its immutable registered identity")
    p8v7_reset_delta = {("D", path) for path in _O8_ADDITIONS}
    p8v7_reset_delta.add(("A", _PREREGISTRATION_DOCUMENT))
    _require_delta(
        root,
        _O8V3_COMMIT,
        _PREREGISTRATION_COMMIT,
    ...
    if listing != expected_listing:
        _fail("P8v7 recovery document Git identity is invalid")
    document_raw = _git(
        root,
        "cat-file",
        "blob",
    ...
    if (
        ...
        _fail("P8v7 recovery document raw identity is invalid")

    _require_direct_child(root, open_commit, _PREREGISTRATION_COMMIT, name="O8v4")
    _require_delta(
        root,
        _PREREGISTRATION_COMMIT,
        open_commit,
...
def _expected_authority_identities(
    ...
    append("raw_audit", ["cat-file", "-t", f"refs/tags/{_P8V6_TAG}"])
    append("raw_audit", ["rev-parse", f"refs/tags/{_P8V6_TAG}"])
    append("raw_audit", ["rev-list", "--parents", "-n", "1", _P8V6_COMMIT])
    append(
        "raw_audit",
        [
            "diff",
    ...
    append("raw_audit", ["cat-file", "-t", f"refs/tags/{_O8V3_TAG}"])
    append("raw_audit", ["rev-parse", f"refs/tags/{_O8V3_TAG}"])
    append("raw_audit", ["rev-list", "--parents", "-n", "1", _O8V3_COMMIT])
    append(
        "raw_audit",
        [
            "diff",
    ...
    if len(commands) != 54:
        _fail("internal authority command plan is not 54 rows")
    return [
        _command_identity(
            attempt_index=None,
            label="authority",
