...
def test_preparation_exclusive_publication_never_replaces_existing_bytes(
    ...
    preparation._publish_bytes_exclusive(destination, b"first", "synthetic-receipt")
    assert destination.read_bytes() == b"first"
    with pytest.raises(preparation.ProtocolError):
        preparation._publish_bytes_exclusive(destination, b"second", "synthetic-receipt")
    assert destination.read_bytes() == b"first"
...
def test_lifecycle_orders_arm_a_validation_b_validation_without_retry_or_third_start(
    ...
    monkeypatch.setattr(lifecycle, "_optional_raw", lambda _path: b"payload")
...
def test_lifecycle_ledger_accepts_only_registered_order_prefixes(
    ...
    monkeypatch.setattr(lifecycle, "_artifact_hash", lambda _path: None)
...
def _publisher_result_transaction(work_root: Path) -> dict[str, Any]:
    ...
    authority = "/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open/authority"
...
def test_publisher_rejects_rehashed_malformed_scientific_receipt_before_r8(
    ...
    def scientific_payload_state(path: Path, _label: str) -> lifecycle._EvidenceState:
...
def test_publisher_o8_identity_requires_exactly_one_parent_equal_to_p8(
    ...
    monkeypatch.setattr(
        lifecycle, "_PREREGISTRATION_DOCUMENT_BLOB", lifecycle._git_oid("blob", p8v6_raw)
    )
    monkeypatch.setattr(
        lifecycle, "_PREREGISTRATION_DOCUMENT_SHA256", hashlib.sha256(p8v6_raw).hexdigest()
    )
    monkeypatch.setattr(lifecycle, "_PREREGISTRATION_DOCUMENT_BYTE_COUNT", len(p8v6_raw))
    ...
    lineage = {
        ...
        lifecycle._PREREGISTRATION_COMMIT: lifecycle._O8V2_COMMIT,
    }
...
def _schema_gap_fallback_fixture(
    ...
    def fixture_evidence_state(
        ...
        *,
        ...
        return real_evidence_state(path, label, maximum=maximum)
...
def test_p8v6_lifecycle_frozen_identity_execution_schema_and_windows_object_parity() -> None:
    document_name = (
        "experiment_amendment_2026-08-18_action_qbc_v8_open_"
        "bounded_remote_verification_v6_runner_manifest_key_recovery.md"
    )
...
def test_p8v6_recovery_authority_plan_adds_eight_rows_and_attempt_plan_stays_54() -> None:
    registration = {
        "source_manifest": {
            "preregistration_tree": [],
            "open_freeze_added_files": [],
...
def _runner_recovery_git_responses(
    ...
    return {
        ...
        ("rev-list", "--parents", "-n", "1", runner._PREREGISTRATION_COMMIT): (
            f"{runner._PREREGISTRATION_COMMIT} {runner._O8V2_COMMIT}\n".encode("ascii")
        ),
...
def test_runner_requires_complete_p8v6_recovery_lineage(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    open_commit = "f" * 40
    document_raw = b"synthetic P8v6 recovery document\n"
    monkeypatch.setattr(runner, "_PREREGISTRATION_DOCUMENT_BYTE_COUNT", len(document_raw))
    monkeypatch.setattr(
        runner,
        "_PREREGISTRATION_DOCUMENT_SHA256",
        hashlib.sha256(document_raw).hexdigest(),
    )
    responses = _runner_recovery_git_responses(open_commit, document_raw)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (runner._O8V2_COMMIT, runner._PREREGISTRATION_COMMIT),
        (runner._P8V5_COMMIT, runner._PREREGISTRATION_COMMIT),
...
def test_runner_rejects_each_inexact_recovery_delta(
    ...
    responses = _runner_recovery_git_responses("f" * 40, document_raw)
...
def test_p8v4_lifecycle_selected_bundle_allows_only_cleanup_failure_coexistence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        lifecycle,
        "_evidence_state",
        lambda path, _label: emergency_state
    ...
    with pytest.raises(lifecycle.LifecycleError, match="sole P8v4 cleanup override"):
...
def _p8v4_semantic_closure_prepare_harness(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> tuple[SimpleNamespace, list[tuple[Path, bytes, str]]]:
    authority = tmp_path / "authority"
    execution_root = tmp_path / "execution"
    authority.mkdir()
    ...
    monkeypatch.setattr(
        ...
        lambda path, raw, name: published.append((path, raw, name)),
    )
...
def test_p8v4_semantic_closure_outer_cleanup_failure_is_typed_and_stops_later_children(
    ...
    monkeypatch.setattr(lifecycle, "_optional_raw", lambda *_a: b"payload")
