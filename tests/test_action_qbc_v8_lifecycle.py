...
def test_publisher_o8_identity_requires_exactly_one_parent_equal_to_p8(
    ...
    monkeypatch.setattr(
        lifecycle, "_PREREGISTRATION_DOCUMENT_BLOB", lifecycle._git_oid("blob", p8v4_raw)
    )
    monkeypatch.setattr(
        lifecycle, "_PREREGISTRATION_DOCUMENT_SHA256", hashlib.sha256(p8v4_raw).hexdigest()
    )
    monkeypatch.setattr(lifecycle, "_PREREGISTRATION_DOCUMENT_BYTE_COUNT", len(p8v4_raw))
    ...
    lineage = {
        ...
        lifecycle._PREREGISTRATION_COMMIT: lifecycle._P8V3_COMMIT,
    }
    ...
    def fake_git(
        ...
        *arguments: str,
        ...
        if arguments == (
            ...
            return b"A\0" + lifecycle._PREREGISTRATION_DOCUMENT.encode("utf-8") + b"\0"
...
def _schema_gap_successful_machine(
    *,
    ...
    remote_receipt: dict[str, Any] = {
        ...
        "ref": "refs/tags/action-qbc-v8-open-diagnostic-freeze-v1",
        "python": {
...
def test_schema_gap_remote_attempt_key_mutations_fail_both_real_validators() -> None:
    expected_stdout = (
        ...
        b"\trefs/tags/action-qbc-v8-open-diagnostic-freeze-v1\n"
    )
...
def test_p8v4_lifecycle_frozen_identity_execution_schema_and_windows_object_parity() -> None:
    document_name = (
        "experiment_amendment_2026-08-11_action_qbc_v8_open_"
        "bounded_remote_verification_v4_correction.md"
    )
    document = (
        ROOT / "docs" / document_name
    ).read_bytes()
    modules = (preparation, lifecycle, finalizer)
    assert {
    ...
    assert [len(module._EXECUTION_KEYS) for module in modules] == [70, 70, 70]
    assert preparation._EXECUTION_KEYS == lifecycle._EXECUTION_KEYS == finalizer._EXECUTION_KEYS
    assert (
...
def test_p8v4_lifecycle_authority_plan_adds_four_rows_and_attempt_plan_stays_54() -> None:
    registration = {
        "source_manifest": {
            "preregistration_tree": [],
    ...
    execution = {"authority_root": "/synthetic/authority"}
    ...
    assert execute_rows == finalizer_rows
    assert len(execute_rows) == 30
    suffixes = [row["argv"][5:] for row in execute_rows]
    assert ["-t", f"refs/tags/{lifecycle._P8V3_TAG}"] in suffixes
    assert [f"refs/tags/{lifecycle._P8V3_TAG}"] in suffixes
    assert ["--parents", "-n", "1", lifecycle._P8V3_COMMIT] in suffixes
    assert [
        "--name-status", "--no-renames", "-z", lifecycle._P8V2_COMMIT,
        lifecycle._P8V3_COMMIT,
    ] in suffixes
