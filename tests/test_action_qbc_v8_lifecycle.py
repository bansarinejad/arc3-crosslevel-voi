...
def test_publisher_o8_identity_requires_exactly_one_parent_equal_to_p8(
    ...
    monkeypatch.setattr(
        lifecycle, "_PREREGISTRATION_DOCUMENT_BLOB", lifecycle._git_oid("blob", p8v5_raw)
    )
    monkeypatch.setattr(
        lifecycle, "_PREREGISTRATION_DOCUMENT_SHA256", hashlib.sha256(p8v5_raw).hexdigest()
    )
    monkeypatch.setattr(lifecycle, "_PREREGISTRATION_DOCUMENT_BYTE_COUNT", len(p8v5_raw))
    ...
    lineage = {
        ...
        lifecycle._PREREGISTRATION_COMMIT: lifecycle._O8V1_COMMIT,
    }
...
def test_p8v5_lifecycle_frozen_identity_execution_schema_and_windows_object_parity() -> None:
    document_name = (
        "experiment_amendment_2026-08-18_action_qbc_v8_open_"
        "bounded_remote_verification_v5_public_visibility_recovery.md"
    )
...
def test_p8v5_recovery_authority_plan_adds_eight_rows_and_attempt_plan_stays_54() -> None:
    registration = {
        "source_manifest": {
            "preregistration_tree": [],
...
def _runner_recovery_git_responses(
    ...
    return {
        ...
        ("rev-list", "--parents", "-n", "1", runner._PREREGISTRATION_COMMIT): (
            f"{runner._PREREGISTRATION_COMMIT} {runner._O8V1_COMMIT}\n".encode("ascii")
        ),
...
def test_runner_requires_complete_p8v5_recovery_lineage(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    open_commit = "f" * 40
    document_raw = b"synthetic P8v5 recovery document\n"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (runner._O8V1_COMMIT, runner._PREREGISTRATION_COMMIT),
        (runner._P8V4_COMMIT, runner._PREREGISTRATION_COMMIT),
