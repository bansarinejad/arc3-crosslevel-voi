...
P8_COMMIT = "15059c482d9e463f01cb31fdfd33c96d1f60db0a"
P8_DOCUMENT_GIT_BLOB_SHA1 = "c0cda2417bd98a42b76e8e1bbdee4cec01dd68f9"
P8_DOCUMENT_SHA256 = (
    "f729904367dd7a2664ecd3fdfe4893841326668fbe892b3b733926ad7840745d"
)
P8V6_COMMIT = "61cebe90a2f4f7c78ec45119de53a482ed13a655"
P8V6_DOCUMENT_SHA256 = (
    "0ba4cc55ca2b31433bc458972ffc32d87f84b610673fa22ed2b4dd4a8bfc1a41"
)
P8V5_COMMIT = "09f9caea346866a1acf35c20e0c9d937096b5ce3"
P8V5_DOCUMENT_SHA256 = (
...
REPLACEMENTS: tuple[tuple[bytes, bytes, int], ...] = (
    ...
    (
        ...
        b"action-qbc-v8-open-diagnostic-freeze-v4",
        1,
    ),
    (
    ...
    (
        ...
        (
            ...
            b"action_qbc_v8_open_bounded_remote_verification_v7_consumed_lifecycle_recovery.md"
        ),
        1,
    ),
    (
...
O8V1_TO_O8V2_ADMINISTRATIVE_REPLACEMENTS: tuple[tuple[bytes, bytes, int], ...] = (
    ...
    (
        ...
        P8V5_DOCUMENT_SHA256.encode("ascii"),
        1,
    ),
)

O8V2_TO_O8V3_ADMINISTRATIVE_REPLACEMENTS: tuple[tuple[bytes, bytes, int], ...] = (
    (
        b"prereg-action-qbc-v8-open-bounded-remote-verification-v5",
        b"prereg-action-qbc-v8-open-bounded-remote-verification-v6",
        1,
    ),
    (
        b"action-qbc-v8-open-diagnostic-freeze-v2",
        b"action-qbc-v8-open-diagnostic-freeze-v3",
        1,
    ),
    (
        P8V5_COMMIT.encode("ascii"),
        P8V6_COMMIT.encode("ascii"),
        1,
    ),
    (
        (
        ...
        (
            b"docs/experiment_amendment_2026-08-18_"
            b"action_qbc_v8_open_bounded_remote_verification_v6_runner_manifest_key_recovery.md"
        ),
        1,
    ),
    (
        P8V5_DOCUMENT_SHA256.encode("ascii"),
        P8V6_DOCUMENT_SHA256.encode("ascii"),
        1,
    ),
)

O8V3_TO_O8V4_ADMINISTRATIVE_REPLACEMENTS: tuple[tuple[bytes, bytes, int], ...] = (
    (
        b"prereg-action-qbc-v8-open-bounded-remote-verification-v6",
        b"prereg-action-qbc-v8-open-bounded-remote-verification-v7",
        1,
...
def test_o8v1_to_o8v2_audit_delta_is_exactly_five_administrative_replacements() -> None:
    ...
    o8_v3 = current
    for old, new, expected_count in reversed(O8V3_TO_O8V4_ADMINISTRATIVE_REPLACEMENTS):
        assert o8_v3.count(new) == expected_count
        o8_v3 = o8_v3.replace(new, old)
    o8_v2 = o8_v3
    for old, new, expected_count in reversed(O8V2_TO_O8V3_ADMINISTRATIVE_REPLACEMENTS):
        assert o8_v2.count(new) == expected_count
        o8_v2 = o8_v2.replace(new, old)
    assert len(o8_v2) == 180_246
    assert _git_blob_sha1(o8_v2) == "002f262ad23589647384bf73491094b80ca35cf4"
    assert _sha256(o8_v2) == (
...
def test_o8v2_to_o8v3_audit_delta_is_exactly_five_administrative_replacements() -> None:
    current = V8_AUDIT_PATH.read_bytes()
    o8_v3 = current
    for old, new, expected_count in reversed(O8V3_TO_O8V4_ADMINISTRATIVE_REPLACEMENTS):
        assert o8_v3.count(new) == expected_count
        o8_v3 = o8_v3.replace(new, old)
    assert len(o8_v3) == 180_248
    assert _git_blob_sha1(o8_v3) == "cf851c9041c56d4a24abda3def1e8a4f78642496"
    assert _sha256(o8_v3) == (
    ...
    for old, new, expected_count in reversed(O8V2_TO_O8V3_ADMINISTRATIVE_REPLACEMENTS):
        assert historical.count(new) == expected_count
        historical = historical.replace(new, old)
    assert len(historical) == 180_246
    assert _git_blob_sha1(historical) == "002f262ad23589647384bf73491094b80ca35cf4"
    assert _sha256(historical) == (
        "59a6b2485fe5dc0293b483d98e0cbdc219fb7a07fb17e2ef8bd8bc8543424a47"
    )
    replay = historical
    for old, new, expected_count in O8V2_TO_O8V3_ADMINISTRATIVE_REPLACEMENTS:
        assert replay.count(old) == expected_count
        replay = replay.replace(old, new)
    assert replay == o8_v3


def test_o8v3_to_o8v4_audit_delta_is_exactly_five_administrative_replacements() -> None:
    current = V8_AUDIT_PATH.read_bytes()
    historical = current
    for old, new, expected_count in reversed(O8V3_TO_O8V4_ADMINISTRATIVE_REPLACEMENTS):
        assert historical.count(new) == expected_count
        historical = historical.replace(new, old)
    assert len(historical) == 180_248
    assert _git_blob_sha1(historical) == "cf851c9041c56d4a24abda3def1e8a4f78642496"
    assert _sha256(historical) == (
        "9d60cb805ddb49f10ca21a02225a9434786c24152d03054feafefb85de12256a"
    )
    replay = historical
    for old, new, expected_count in O8V3_TO_O8V4_ADMINISTRATIVE_REPLACEMENTS:
        assert replay.count(old) == expected_count
        replay = replay.replace(old, new)
...
def test_v8_identity_constants_are_truthful_and_scientific_names_stay_frozen() -> None:
    ...
    assert audit.OPEN_FREEZE_TAG == "action-qbc-v8-open-diagnostic-freeze-v4"
