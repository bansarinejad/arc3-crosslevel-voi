...
def test_exact_frozen_identity_and_allowlist_constants() -> None:
    assert reconstruction.PREREGISTRATION_TAG == "prereg-action-qbc-v8-open-bounded-remote-verification-v5"
    assert reconstruction.PREREGISTRATION_COMMIT == "09f9caea346866a1acf35c20e0c9d937096b5ce3"
    assert reconstruction.PREREGISTRATION_DOCUMENT_BLOB == "7c0955a775af89dcfcde4796a9bbb4d470669d10"
    assert reconstruction.PREREGISTRATION_DOCUMENT_SHA256 == "cc9d787a64700332a44f543e7a949ee5522c3663b6b0eb54e418840e560cfe6d"
    assert reconstruction.PREREGISTRATION_DOCUMENT_BYTE_COUNT == 25_872
    ...
    assert reconstruction.OPEN_FREEZE_TAG == "action-qbc-v8-open-diagnostic-freeze-v2"
    assert reconstruction.RESULT_TAG == "action-qbc-v8-open-diagnostic-result-v2"
...
def test_preregistration_and_v7_git_object_anchors_are_exact() -> None:
    ...
    p8_v5_entries = reconstruction._tree_entries(ROOT, reconstruction.PREREGISTRATION_COMMIT)
    assert [entry for entry in p8_v5_entries if entry[0] != reconstruction.PREREGISTRATION_DOCUMENT] == p8_v4_entries
...
def test_audit_replacement_table_and_generated_bytes_are_exact() -> None:
    ...
    assert len(transformed) == 180246
    assert hashlib.sha256(transformed).hexdigest() == "59a6b2485fe5dc0293b483d98e0cbdc219fb7a07fb17e2ef8bd8bc8543424a47"
    assert hashlib.sha1(
        b"blob " + str(len(transformed)).encode() + b"\0" + transformed,
        usedforsecurity=False,
    ).hexdigest() == "002f262ad23589647384bf73491094b80ca35cf4"
...
def test_authority_ledger_binds_complete_r7_through_p8v5_o8v1_lineage() -> None:
    ...
    assert tag_positions == [4, 8, 12, 16, 20, 24]
    for parent, child in (
...
def test_runbook_closes_exact_pre_o8_operator_and_publication_contract() -> None:
    ...
    section_one = runbook.split("## 1. Build and freeze O8v2", 1)[1].split(
    ...
    assert "a partial or failed sequence is not O8v2" in section_one
    ...
    visibility_change = [
        ...
        "repo",
        "edit",
        "bansarinejad/arc3-crosslevel-voi",
        "--visibility",
        "public",
        "--accept-visibility-change-consequences",
    ]
    for argv in (atomic_push, visibility_change, independent_https_check):
    ...
    assert "repository must remain PRIVATE through that successful atomic push" in section_one
    assert "sole authorized PRIVATE-to-PUBLIC transition" in section_one
    assert "path:        C:\\Program Files\\GitHub CLI\\gh.exe" in section_one
    assert "version:     2.96.0" in section_one
    assert "byte count:  41504056" in section_one
    assert "cd79f16203f1fbe56937c4c96e2b6eadd10549418dcb241d91576ac77af0ac8b" in section_one
    assert section_one.index(reconstruction.canonical_json_bytes(atomic_push).decode("ascii")) < section_one.index(
        reconstruction.canonical_json_bytes(visibility_change).decode("ascii")
    ) < section_one.index(reconstruction.canonical_json_bytes(independent_https_check).decode("ascii"))
...
def _full_preparation_ledger(
    ...
    *,
    ...
    assert len(authority_identities) == 38
