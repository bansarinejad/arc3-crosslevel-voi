...
def test_builder_remains_byte_identical_to_historical_o8v1() -> None:
    relative = "scripts/build_action_qbc_v8_open_registration.py"
    historical_entry, historical_raw = reconstruction._blob_at(
        ROOT,
        reconstruction.OPEN_FREEZE_V1_COMMIT,
        relative,
    )
    current = (ROOT / relative).read_bytes()
    assert current == historical_raw
    assert historical_entry["sha256"] == hashlib.sha256(current).hexdigest()
    assert historical_entry["sha256"] == (
        "389636cbd53c20fda4606b56db0212bca7fe29a87c9c05ceec49f2195a80ffbc"
    )


def test_exact_frozen_identity_and_allowlist_constants() -> None:
    assert reconstruction.PREREGISTRATION_TAG == "prereg-action-qbc-v8-open-bounded-remote-verification-v7"
    assert reconstruction.PREREGISTRATION_COMMIT == "15059c482d9e463f01cb31fdfd33c96d1f60db0a"
    assert reconstruction.PREREGISTRATION_TREE == "96469ca9ee018cd32f99955df1ded57af12a8abc"
    assert reconstruction.PREREGISTRATION_DOCUMENT_BLOB == "c0cda2417bd98a42b76e8e1bbdee4cec01dd68f9"
    assert reconstruction.PREREGISTRATION_DOCUMENT_SHA256 == "f729904367dd7a2664ecd3fdfe4893841326668fbe892b3b733926ad7840745d"
    assert reconstruction.PREREGISTRATION_DOCUMENT_BYTE_COUNT == 37_552
    assert reconstruction.PREREGISTRATION_V6_TAG == "prereg-action-qbc-v8-open-bounded-remote-verification-v6"
    assert reconstruction.PREREGISTRATION_V6_COMMIT == "61cebe90a2f4f7c78ec45119de53a482ed13a655"
    assert reconstruction.PREREGISTRATION_V6_TREE == "65695876c44eeb8cac5437149384071f88ff6018"
    assert reconstruction.PREREGISTRATION_V6_DOCUMENT_BLOB == "5e870ed0bbbff6fcb4352f6e914d870254773f68"
    assert reconstruction.PREREGISTRATION_V6_DOCUMENT_SHA256 == "0ba4cc55ca2b31433bc458972ffc32d87f84b610673fa22ed2b4dd4a8bfc1a41"
    assert reconstruction.PREREGISTRATION_V6_DOCUMENT_BYTE_COUNT == 32_370
    assert reconstruction.PREREGISTRATION_V5_TAG == "prereg-action-qbc-v8-open-bounded-remote-verification-v5"
    assert reconstruction.PREREGISTRATION_V5_COMMIT == "09f9caea346866a1acf35c20e0c9d937096b5ce3"
    assert reconstruction.PREREGISTRATION_V5_TREE == "47a978cdd887fd6dc1cb5e80e36aa3e0a5a29253"
    assert reconstruction.PREREGISTRATION_V5_DOCUMENT_BLOB == "7c0955a775af89dcfcde4796a9bbb4d470669d10"
    assert reconstruction.PREREGISTRATION_V5_DOCUMENT_SHA256 == "cc9d787a64700332a44f543e7a949ee5522c3663b6b0eb54e418840e560cfe6d"
    assert reconstruction.PREREGISTRATION_V5_DOCUMENT_BYTE_COUNT == 25_872
    assert reconstruction.PREREGISTRATION_V4_TAG == "prereg-action-qbc-v8-open-bounded-remote-verification-v4"
    assert reconstruction.PREREGISTRATION_V4_COMMIT == "e0bff9ffc185196cafa938c8f7c9a7186366258b"
    assert reconstruction.PREREGISTRATION_V4_DOCUMENT_BLOB == "29c991b7e23209f2c38d5e9a11a15bca51753d8e"
    assert reconstruction.PREREGISTRATION_V4_DOCUMENT_SHA256 == "31d6a04b113e5f18621c3b27af69d9e7d3a19289047673719ccd149d33b5b7b1"
    assert reconstruction.PREREGISTRATION_V4_DOCUMENT_BYTE_COUNT == 33_215
    ...
    assert reconstruction.OPEN_FREEZE_V1_COMMIT == "7685fbdccd41702216b3a3f06d2a0ac699aca7ec"
    assert reconstruction.OPEN_FREEZE_V1_TAG == "action-qbc-v8-open-diagnostic-freeze-v1"
    assert reconstruction.OPEN_FREEZE_V1_TREE == "9b9ad5ba986afacbcdb1fde3cd69e0f1c94efdf2"
    assert reconstruction.OPEN_FREEZE_V2_COMMIT == "8da637a47de0c88f917f222e52e54b342d729be9"
    assert reconstruction.OPEN_FREEZE_V2_TAG == "action-qbc-v8-open-diagnostic-freeze-v2"
    assert reconstruction.OPEN_FREEZE_V2_TREE == "247eba59e1e2ac9b0611c0e361de945dae0f2dc8"
    assert reconstruction.OPEN_FREEZE_V3_COMMIT == "5725395a850627fae10e8bb8b27083ccf63b6ec7"
    assert reconstruction.OPEN_FREEZE_V3_TAG == "action-qbc-v8-open-diagnostic-freeze-v3"
    assert reconstruction.OPEN_FREEZE_V3_TREE == "7d38de8f5cec16cab92c9d3b757a218e8e490272"
...
def test_preregistration_and_v7_git_object_anchors_are_exact() -> None:
    ...
    assert _git(ROOT, "cat-file", "-t", reconstruction.PREREGISTRATION_V4_TAG).strip() == b"commit"
    assert _git(ROOT, "rev-parse", reconstruction.PREREGISTRATION_V4_TAG).decode().strip() == reconstruction.PREREGISTRATION_V4_COMMIT
    assert _git(ROOT, "cat-file", "-t", reconstruction.OPEN_FREEZE_V1_TAG).strip() == b"commit"
    assert _git(ROOT, "rev-parse", reconstruction.OPEN_FREEZE_V1_TAG).decode().strip() == reconstruction.OPEN_FREEZE_V1_COMMIT
    assert _git(ROOT, "cat-file", "-t", reconstruction.PREREGISTRATION_V5_TAG).strip() == b"commit"
    assert _git(ROOT, "rev-parse", reconstruction.PREREGISTRATION_V5_TAG).decode().strip() == reconstruction.PREREGISTRATION_V5_COMMIT
    assert _git(ROOT, "cat-file", "-t", reconstruction.OPEN_FREEZE_V2_TAG).strip() == b"commit"
    assert _git(ROOT, "rev-parse", reconstruction.OPEN_FREEZE_V2_TAG).decode().strip() == reconstruction.OPEN_FREEZE_V2_COMMIT
    assert _git(ROOT, "cat-file", "-t", reconstruction.PREREGISTRATION_V6_TAG).strip() == b"commit"
    assert _git(ROOT, "rev-parse", reconstruction.PREREGISTRATION_V6_TAG).decode().strip() == reconstruction.PREREGISTRATION_V6_COMMIT
    assert _git(ROOT, "cat-file", "-t", reconstruction.OPEN_FREEZE_V3_TAG).strip() == b"commit"
    assert _git(ROOT, "rev-parse", reconstruction.OPEN_FREEZE_V3_TAG).decode().strip() == reconstruction.OPEN_FREEZE_V3_COMMIT
    ...
    assert _git(
        ...
        "-n",
        "1",
        reconstruction.PREREGISTRATION_V4_COMMIT,
    ) == (
        f"{reconstruction.PREREGISTRATION_V4_COMMIT} "
        f"{reconstruction.PREREGISTRATION_V3_COMMIT}\n"
    ).encode("ascii")
    assert _git(
        ROOT,
        ...
        f"{reconstruction.OPEN_FREEZE_V1_COMMIT} "
        f"{reconstruction.PREREGISTRATION_V4_COMMIT}\n"
    ).encode("ascii")
    assert _git(
        ROOT,
        ...
        "-n",
        "1",
        reconstruction.PREREGISTRATION_V5_COMMIT,
    ) == (
        f"{reconstruction.PREREGISTRATION_V5_COMMIT} "
        f"{reconstruction.OPEN_FREEZE_V1_COMMIT}\n"
    ).encode("ascii")
    assert _git(
        ROOT,
        ...
        "-n",
        "1",
        reconstruction.OPEN_FREEZE_V2_COMMIT,
    ) == (
        f"{reconstruction.OPEN_FREEZE_V2_COMMIT} "
        f"{reconstruction.PREREGISTRATION_V5_COMMIT}\n"
    ).encode("ascii")
    assert _git(
        ROOT,
        ...
        "-n",
        "1",
        reconstruction.PREREGISTRATION_V6_COMMIT,
    ) == (
        f"{reconstruction.PREREGISTRATION_V6_COMMIT} "
        f"{reconstruction.OPEN_FREEZE_V2_COMMIT}\n"
    ).encode("ascii")
    assert _git(
        ROOT,
        ...
        "-n",
        "1",
        reconstruction.OPEN_FREEZE_V3_COMMIT,
    ) == (
        f"{reconstruction.OPEN_FREEZE_V3_COMMIT} "
        f"{reconstruction.PREREGISTRATION_V6_COMMIT}\n"
    ).encode("ascii")
    assert _git(
        ROOT,
    ...
    v4_document = reconstruction.PREREGISTRATION_V4_DOCUMENT.encode("utf-8")
    assert _git(
        ROOT,
        "diff",
        "--name-status",
    ...
    assert _git(
        ...
        reconstruction.PREREGISTRATION_V4_COMMIT,
        reconstruction.OPEN_FREEZE_V1_COMMIT,
    ) == reconstruction._expected_name_status(reconstruction.ALL_ADDITIONS)
    assert _git(
        ROOT,
        "diff",
        "--name-status",
        ...
        reconstruction.OPEN_FREEZE_V1_COMMIT,
        reconstruction.PREREGISTRATION_V5_COMMIT,
    ) == reconstruction._expected_typed_name_status(
        [
            *(("D", path) for path in reconstruction.ALL_ADDITIONS),
            ("A", reconstruction.PREREGISTRATION_V5_DOCUMENT),
        ]
    )
    assert _git(
        ROOT,
    ...
    assert _git(
        ...
        [
            *(("D", path) for path in reconstruction.ALL_ADDITIONS),
            ("A", reconstruction.PREREGISTRATION_V6_DOCUMENT),
        ]
    )
    assert _git(
        ROOT,
        ...
        reconstruction.PREREGISTRATION_V5_COMMIT,
        reconstruction.PREREGISTRATION_V6_COMMIT,
    ) == b"A\0" + reconstruction.PREREGISTRATION_V6_DOCUMENT.encode("utf-8") + b"\0"
    assert _git(
        ROOT,
        "diff",
        "--name-status",
    ...
    assert _git(
        ...
        [
            *(("D", path) for path in reconstruction.ALL_ADDITIONS),
            ("A", reconstruction.PREREGISTRATION_DOCUMENT),
        ]
    )
    assert _git(
        ROOT,
        ...
        reconstruction.PREREGISTRATION_V6_COMMIT,
        reconstruction.PREREGISTRATION_COMMIT,
    ) == b"A\0" + reconstruction.PREREGISTRATION_DOCUMENT.encode("utf-8") + b"\0"
    ...
    v4_entry, v4_raw = reconstruction._blob_at(
        ROOT,
        reconstruction.PREREGISTRATION_V4_COMMIT,
        reconstruction.PREREGISTRATION_V4_DOCUMENT,
    )
    assert v4_entry["git_blob_sha1"] == reconstruction.PREREGISTRATION_V4_DOCUMENT_BLOB
    assert hashlib.sha256(v4_raw).hexdigest() == reconstruction.PREREGISTRATION_V4_DOCUMENT_SHA256
    v5_entry, v5_raw = reconstruction._blob_at(
        ROOT,
        reconstruction.PREREGISTRATION_V5_COMMIT,
        reconstruction.PREREGISTRATION_V5_DOCUMENT,
    )
    assert v5_entry["git_blob_sha1"] == reconstruction.PREREGISTRATION_V5_DOCUMENT_BLOB
    assert hashlib.sha256(v5_raw).hexdigest() == reconstruction.PREREGISTRATION_V5_DOCUMENT_SHA256
    assert len(v5_raw) == reconstruction.PREREGISTRATION_V5_DOCUMENT_BYTE_COUNT
    v6_entry, v6_raw = reconstruction._blob_at(
        ROOT,
        reconstruction.PREREGISTRATION_V6_COMMIT,
        reconstruction.PREREGISTRATION_V6_DOCUMENT,
    )
    assert v6_entry["git_blob_sha1"] == reconstruction.PREREGISTRATION_V6_DOCUMENT_BLOB
    assert hashlib.sha256(v6_raw).hexdigest() == reconstruction.PREREGISTRATION_V6_DOCUMENT_SHA256
    assert len(v6_raw) == reconstruction.PREREGISTRATION_V6_DOCUMENT_BYTE_COUNT
    ...
    assert _git(
        ROOT, "rev-parse", f"{reconstruction.OPEN_FREEZE_V1_COMMIT}^{{tree}}"
    ).decode("ascii").strip() == reconstruction.OPEN_FREEZE_V1_TREE
    assert _git(
        ROOT, "rev-parse", f"{reconstruction.PREREGISTRATION_V5_COMMIT}^{{tree}}"
    ).decode("ascii").strip() == reconstruction.PREREGISTRATION_V5_TREE
    assert _git(
        ROOT, "rev-parse", f"{reconstruction.OPEN_FREEZE_V2_COMMIT}^{{tree}}"
    ).decode("ascii").strip() == reconstruction.OPEN_FREEZE_V2_TREE
    assert _git(
        ROOT, "rev-parse", f"{reconstruction.PREREGISTRATION_V6_COMMIT}^{{tree}}"
    ).decode("ascii").strip() == reconstruction.PREREGISTRATION_V6_TREE
    assert _git(
        ROOT, "rev-parse", f"{reconstruction.OPEN_FREEZE_V3_COMMIT}^{{tree}}"
    ).decode("ascii").strip() == reconstruction.OPEN_FREEZE_V3_TREE
    assert _git(
        ROOT, "rev-parse", f"{reconstruction.PREREGISTRATION_COMMIT}^{{tree}}"
    ).decode("ascii").strip() == reconstruction.PREREGISTRATION_TREE
    p8_v4_entries = reconstruction._tree_entries(
        ROOT, reconstruction.PREREGISTRATION_V4_COMMIT
    )
    p8_v5_entries = reconstruction._tree_entries(ROOT, reconstruction.PREREGISTRATION_V5_COMMIT)
    assert [entry for entry in p8_v5_entries if entry[0] != reconstruction.PREREGISTRATION_V5_DOCUMENT] == p8_v4_entries
    p8_v6_entries = reconstruction._tree_entries(
        ROOT, reconstruction.PREREGISTRATION_V6_COMMIT
    )
    assert [
        entry
        for entry in p8_v6_entries
        if entry[0] != reconstruction.PREREGISTRATION_V6_DOCUMENT
    ] == p8_v5_entries
    p8_v7_entries = reconstruction._tree_entries(ROOT, reconstruction.PREREGISTRATION_COMMIT)
    assert len(p8_v7_entries) == 233
    assert [
        entry
        for entry in p8_v7_entries
        if entry[0] != reconstruction.PREREGISTRATION_DOCUMENT
    ] == p8_v6_entries
...
def test_audit_replacement_table_and_generated_bytes_are_exact() -> None:
    ...
    assert len(transformed) == 180247
    assert hashlib.sha256(transformed).hexdigest() == "ac9ae39b7aebe8a67a7914c300c3c8478ab414146a69f8599c2cbd271c4e8e74"
    assert hashlib.sha1(
        b"blob " + str(len(transformed)).encode() + b"\0" + transformed,
        usedforsecurity=False,
    ).hexdigest() == "e998ea74fe79d25b547e952b1442e2978f09d8db"
...
def test_execution_contract_has_exact_keys_commands_hashes_and_remote_policy() -> None:
    ...
    assert len(execution["result_publisher_argv"]) == 23
    assert execution["result_publisher_argv"][:4] == [
        "/usr/bin/timeout", "--signal=TERM", "--kill-after=5s", "600s",
    ]
    assert execution["result_publisher_argv"][4:8] == [
        "/usr/bin/python3", "-I", "-B", "scripts/execute_action_qbc_v8_open_lifecycle.py",
    ]
    assert len(execution["result_publisher_argv"][4:]) == 19
    assert execution["result_publisher_argv"][-2:] == ["--control-time-seconds", "570"]
    assert execution["execution_root"] == (
        "/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open-v4"
    )
    assert execution["remote_claim_windows_path"].endswith("-claim-v4.json")
    assert execution["remote_verifier_claim_windows_path"].endswith(
        "-start-claim-v4.json"
    )
    assert execution["remote_receipt_windows_path"].endswith("-verification-v4.json")
    assert execution["remote_supervisor_receipt_windows_path"].endswith(
        "-supervisor-v4.json"
    )
    for key in (
        ...
        assert execution[key] not in {
            ...
            r"D:\kaggle competitions\arc3-crosslevel-voi-action-qbc-v8-remote-verification-supervisor.json",
        }
    ...
    linux_tools = {row["path"]: row for row in execution["linux_tool_identities"]}
    ...
    historical = json.loads(
        _git(
            ROOT,
            "show",
            f"{reconstruction.OPEN_FREEZE_V3_COMMIT}:{reconstruction.REGISTRATION_PATH}",
        )
    )["execution_contract"]["argv_hashes"]
    changed_hashes = {
        key for key, value in execution["argv_hashes"].items() if historical[key] != value
    }
    ...
    assert set(execution["argv_hashes"]) - changed_hashes == {
        "environment_build",
        "linux_host_launcher",
        "preflight",
        "reconstructor",
...
def test_evidence_mode_and_path_maps_are_exhaustive_and_identical() -> None:
    execution = reconstruction._execution_contract(_synthetic_finalizer_manifest())
    immutable = {
        "preparation_receipt",
        "remote_claim",
    ...
    expected_modes = {
        **{role: 0o444 for role in immutable},
        **{role: 0o600 for role in private},
    }
    expected_paths = {
        "preparation_receipt": execution["preparation_receipt_path"],
        "remote_claim": f'{execution["execution_root"]}/remote-verification-claim.json',
        "remote_verifier_claim": (
            f'{execution["execution_root"]}/remote-verifier-start-claim.json'
        ),
        "remote_receipt": f'{execution["execution_root"]}/remote-verification.json',
        "remote_supervisor_receipt": (
            f'{execution["execution_root"]}/remote-verification-supervisor.json'
        ),
        "arm_receipt": execution["arm_receipt_path"],
        "preparation_verification_receipt": execution[
            "preparation_verification_receipt_path"
        ],
        "lifecycle_driver_claim": execution["lifecycle_driver_claim_path"],
        "lifecycle_ledger": execution["lifecycle_ledger_path"],
        "process_a_start_claim": execution["process_a_start_claim"],
        "process_b_start_claim": execution["process_b_start_claim"],
        "process_a_validator_claim": execution["process_a_validator_claim"],
        "process_b_validator_claim": execution["process_b_validator_claim"],
        "process_a_validation_receipt": execution[
            "process_a_validation_receipt"
        ],
        "process_b_validation_receipt": execution[
            "process_b_validation_receipt"
        ],
        "process_a_payload": execution["process_a_output"],
        "process_b_payload": execution["process_b_output"],
        "normal_finalization_bundle": execution["finalization_bundle_path"],
        "emergency_result_bundle": execution["emergency_bundle_path"],
        "result_git_owner_claim": execution["result_git_owner_path"],
    }
    assert len(expected_modes) == len(expected_paths) == 20
    for module in (preparation, lifecycle, finalizer):
        assert expected_modes == module._EVIDENCE_EXPECTED_MODES
        observed_paths = {
            role: str(path).replace("\\", "/")
            for role, path in module._EVIDENCE_PATHS_BY_ROLE.items()
        }
...
def test_authority_ledger_binds_complete_r7_through_p8v7_o8v3_lineage() -> None:
    ...
    prefix = ["/usr/bin/git", "--no-replace-objects", "-C", str(authority)]
    assert argvs[1:4] == [
        [*prefix, "cat-file", "-t", f"refs/tags/{reconstruction.OPEN_FREEZE_TAG}"],
        [*prefix, "rev-parse", f"refs/tags/{reconstruction.OPEN_FREEZE_TAG}"],
    ...
    tag_positions = [
        ...
        for tag in (
            ...
            reconstruction.PREREGISTRATION_V4_TAG,
            reconstruction.OPEN_FREEZE_V1_TAG,
            reconstruction.PREREGISTRATION_V5_TAG,
            reconstruction.OPEN_FREEZE_V2_TAG,
            reconstruction.PREREGISTRATION_V6_TAG,
            reconstruction.OPEN_FREEZE_V3_TAG,
            reconstruction.PREREGISTRATION_TAG,
        )
    ]
    assert tag_positions == [4, 8, 12, 16, 20, 24, 28, 32, 36, 40]
    for parent, child in (
    ...
    assert argvs[-7] == [
        *prefix, "diff", "--name-status", "--no-renames", "-z",
        reconstruction.PREREGISTRATION_COMMIT, "a" * 40,
    ]
    assert argvs[44:46] == [
        [
            *prefix, "ls-tree", "-z", reconstruction.PREREGISTRATION_COMMIT,
            "--", reconstruction.PREREGISTRATION_DOCUMENT,
        ],
        [
            *prefix, "cat-file", "blob",
            f"{reconstruction.PREREGISTRATION_COMMIT}:{reconstruction.PREREGISTRATION_DOCUMENT}",
        ],
    ]
...
def test_runbook_closes_exact_pre_o8_operator_and_publication_contract() -> None:
    ...
    section_one = runbook.split("## 1. Build and freeze O8v4", 1)[1].split(
        "## 2. Read-only host preflight", 1
    )[0]

    assert "historical O8v1 atomic publication succeeded" in runbook
    assert "exit code 128" in runbook
    assert "zero stdout bytes" in runbook
    assert "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" in runbook
    assert "exactly 83\nASCII stderr bytes with a final LF" in runbook
    assert "2e554a39e2bb4414b5e2c3f5a90ebc87b405b5b8231d692704ec6df4580e8f60" in runbook
    assert "fatal: could not read Username for 'https://github.com': terminal prompts disabled\\n" in runbook
    assert "No O8v1 bootstrap, execution root, claim, receipt, science, result object, or result ref\nexists" in runbook
    assert "Historical O8v2's one-shot atomic publication" in runbook
    assert "successfully verified by its sole anonymous check" in runbook
    assert "exactly two LF-terminated ASCII records (164 bytes" in runbook
    assert "f83b6bd5dfe2dd611a8150559700dd34a3e492c34e14e119d155810c00bdd568" in runbook
    assert "No O8v2\nbootstrap command, execution root, claim, receipt, preparation, lifecycle, science" in runbook
    assert "Historical O8v3 was constructed from P8v6 as exact A15" in runbook
    assert "a64dec2a14b7be5d8da218f86a4c438fbbbc0273f8d9979f9c38b2ade6e1cdbf" in runbook
    assert "canonical failed receipt with zero\nattempts" in runbook
    assert "No scientific start/import, payload, normal bundle, result Git owner/work root" in runbook
    assert "b3540fa1bf09dd6303bdef36001faa475002b9a03128a3602b19535689de026b" in runbook
    assert "097c92383e19a91cc2447afbcca3946cae16690743783863b29eb4a2aa21584c" in runbook
    assert "publishes and\nvalidates its exclusive start claim before taking" in runbook
    assert "one exhaustive expected-mode map" in runbook
    assert "one registered 23-token argv" in runbook
    assert "derived 19-token\nchild-visible suffix" in runbook
    assert "authority plan is exactly 54 rows" in runbook
    ...
    assert "a partial or failed sequence is not O8v4" in section_one
    ...
    p8v7_push = [
        ...
        (
            "refs/tags/prereg-action-qbc-v8-open-bounded-remote-verification-v7:"
            "refs/tags/prereg-action-qbc-v8-open-bounded-remote-verification-v7"
        ),
    ]
    o8v4_push = [
        ...
        "refs/heads/action-qbc-v8-prereg:refs/heads/action-qbc-v8-prereg",
        (
            "refs/tags/action-qbc-v8-open-diagnostic-freeze-v4:"
            "refs/tags/action-qbc-v8-open-diagnostic-freeze-v4"
        ),
    ]
    ...
    for argv in (p8v7_push, o8v4_push, independent_https_check):
    ...
    assert "repository is already PUBLIC and must remain PUBLIC" in section_one
    assert "No visibility invocation,\nvisibility query, or visibility mutation is authorized" in section_one
    assert "gh.exe" not in section_one
    assert "--visibility" not in section_one
    assert section_one.index(
        reconstruction.canonical_json_bytes(p8v7_push).decode("ascii")
    ) < section_one.index(
        reconstruction.canonical_json_bytes(o8v4_push).decode("ascii")
    ) < section_one.index(
        reconstruction.canonical_json_bytes(independent_https_check).decode("ascii")
    )
    assert "No visibility operation,\nconnectivity probe" in section_one
    assert "exact set must be the two requested\nrefs, each once" in section_one
    assert "Neither atomic push nor this independent read-only\ncheck is a frozen scientific command" in section_one
...
def test_exact_candidate_registration_is_accepted_by_every_contract_consumer(
    ...
    assert "added_in_open_freeze" not in source_manifest
    ...
    assert lifecycle._require_registered_execution(
        ...
        publish_argv=execution["result_publisher_argv"][4:],
    ) == execution
...
def test_prepare_attempt_budget_and_cleanup_gate_are_fail_closed(
    ...
    evidence_paths = dict(preparation._EVIDENCE_PATHS_BY_ROLE)
    evidence_paths["preparation_receipt"] = receipt_path
    monkeypatch.setattr(preparation, "_EVIDENCE_PATHS_BY_ROLE", evidence_paths)
...
def test_arm_copies_all_four_validated_sources_once_as_immutable_bytes(
    ...
    evidence_paths.update(
        {
            "remote_claim": tmp_path / "remote-verification-claim.json",
            "remote_verifier_claim": tmp_path / "remote-verifier-start-claim.json",
            "remote_receipt": tmp_path / "remote-verification.json",
            "remote_supervisor_receipt": (
                tmp_path / "remote-verification-supervisor.json"
            ),
...
def test_all_production_v7_literals_and_path_reads_are_exactly_allowlisted() -> None:
    ...
    assert literals["scripts/finalize_action_qbc_v8_open_diagnostic.py"] == {
        "O8v3/P8v7 consumed-lifecycle recovery identity is invalid",
        "P8v7 recovery document byte identity is invalid",
        ...
        "registration Windows repository contract differs from P8v7",
        *recovery_v7_literals,
        *windows_repository_v7_literals,
    }
    assert literals["scripts/execute_action_qbc_v8_open_lifecycle.py"] == {
        "O8v3..P8v7 is not the exact consumed-lifecycle forward reset",
        "O8v4 is not an exact one-parent direct child of P8v7",
        "P8v7 is not the exact direct child of O8v3",
        "P8v7 lightweight tag identity is invalid",
        "P8v7 recovery document byte identity is invalid",
        "P8v7 tree identity is invalid",
        "P8v7 tree is not the P8v6 tree plus the recovery document",
        "P8v7..O8v4 is not the exact fifteen-path open-freeze delta",
        "embedded remote duration violates the exact P8v7 lower bound",
        "normal/emergency coexistence is not the sole P8v7 cleanup override",
        "registered Windows repository contract differs from P8v7",
        "registration P8v7 identity is invalid",
        "registration execution contract does not have exactly the P8v7 70-key schema",
        *recovery_v7_literals,
        *windows_repository_v7_literals,
    }
    assert literals["scripts/prepare_action_qbc_v8_open.py"] == {
        "O8v3..P8v7 is not the exact consumed-lifecycle forward reset",
        "O8v4 is not a direct child of P8v7",
        "P8v7 is not a direct child of O8v3",
        "P8v7 recovery document Git object identity is invalid",
        "P8v7 recovery document raw bytes are invalid",
        "P8v7 tag differs from the frozen consumed-lifecycle recovery commit",
        "P8v7 tag is absent or annotated",
        "P8v7..O8v4 is not the exact fifteen-path open-freeze delta",
        "local Git config differs from the closed P8v7 mapping: ",
        "preparation verification receipt does not match live P8v7 state",
        "registration Windows repository contract differs from P8v7",
        "registration execution contract does not have exactly the P8v7 70-key schema",
        "registration preparation command environment differs from P8v7",
        "registration preparation command policy differs from P8v7",
        "registration preparation-verification argv differs from P8v7",
        *recovery_v7_literals,
        *windows_repository_v7_literals,
    }
    assert literals["scripts/run_action_qbc_v8_open_diagnostic.py"] == {
        "O8v3-to-P8v7",
        "P8v6-to-P8v7",
        "P8v7",
        "P8v7 recovery document Git identity is invalid",
        "P8v7 recovery document raw identity is invalid",
        "P8v7 tag",
        "P8v7 tree differs from its immutable registered identity",
        "P8v7-to-O8v4",
        *recovery_v7_literals,
        *windows_repository_v7_literals,
    }
    ...
    assert literals["scripts/reconstruct_action_qbc_v8_open_registration.py"] == {
        "O8v3..P8v7 is not the exact fifteen-deletion/one-addition reset",
        "P8v6..P8v7 is not the one-document net addition",
        "P8v7",
        "P8v7 document identity differs from the frozen recovery",
        "P8v7 recovery document is absent from its tree",
        "P8v7 tree identity differs",
        "P8v7 tree is not exactly P8v6 plus the recovery document",
        "P8v7..O8v4 is not the exact fifteen-addition allowlist",
    ...
    assert hashlib.sha256(audit_source).hexdigest() == (
        "ac9ae39b7aebe8a67a7914c300c3c8478ab414146a69f8599c2cbd271c4e8e74"
    )
    audit_imports = {
        alias.name
...
def _full_preparation_ledger(
    ...
    *,
    ...
    authority_identities = reconstruction._expected_authority_identities(
        authority,
        open_commit,
        tree_entries,
    )
    attempt_identities = reconstruction._expected_attempt_identities(
        execution,
        execution_root,
        attempt_index,
        open_commit,
    ...
    assert len(authority_identities) == 54
    assert len(attempt_identities) == 54
    identities = [*authority_identities, *attempt_identities]
