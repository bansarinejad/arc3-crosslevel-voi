...
def test_exact_frozen_identity_and_allowlist_constants() -> None:
    assert reconstruction.PREREGISTRATION_TAG == "prereg-action-qbc-v8-open-bounded-remote-verification-v6"
    assert reconstruction.PREREGISTRATION_COMMIT == "61cebe90a2f4f7c78ec45119de53a482ed13a655"
    assert reconstruction.PREREGISTRATION_TREE == "65695876c44eeb8cac5437149384071f88ff6018"
    assert reconstruction.PREREGISTRATION_DOCUMENT_BLOB == "5e870ed0bbbff6fcb4352f6e914d870254773f68"
    assert reconstruction.PREREGISTRATION_DOCUMENT_SHA256 == "0ba4cc55ca2b31433bc458972ffc32d87f84b610673fa22ed2b4dd4a8bfc1a41"
    assert reconstruction.PREREGISTRATION_DOCUMENT_BYTE_COUNT == 32_370
    ...
    assert reconstruction.OPEN_FREEZE_TAG == "action-qbc-v8-open-diagnostic-freeze-v3"
    assert reconstruction.RESULT_TAG == "action-qbc-v8-open-diagnostic-result-v3"
...
def test_preregistration_and_v7_git_object_anchors_are_exact() -> None:
    ...
    p8_v6_entries = reconstruction._tree_entries(ROOT, reconstruction.PREREGISTRATION_COMMIT)
    assert [entry for entry in p8_v6_entries if entry[0] != reconstruction.PREREGISTRATION_DOCUMENT] == p8_v5_entries
...
def test_audit_replacement_table_and_generated_bytes_are_exact() -> None:
    ...
    assert len(transformed) == 180248
    assert hashlib.sha256(transformed).hexdigest() == "9d60cb805ddb49f10ca21a02225a9434786c24152d03054feafefb85de12256a"
    assert hashlib.sha1(
        b"blob " + str(len(transformed)).encode() + b"\0" + transformed,
        usedforsecurity=False,
    ).hexdigest() == "cf851c9041c56d4a24abda3def1e8a4f78642496"
...
def test_execution_contract_has_exact_keys_commands_hashes_and_remote_policy() -> None:
    ...
    historical = json.loads(
        _git(
            ROOT,
            "show",
            f"{reconstruction.OPEN_FREEZE_V2_COMMIT}:{reconstruction.REGISTRATION_PATH}",
        )
    )["execution_contract"]["argv_hashes"]
    changed_hashes = {
        key for key, value in execution["argv_hashes"].items() if historical[key] != value
    }
    assert changed_hashes == {"bootstrap", "producer", "result_ref_transaction"}
...
def test_authority_ledger_binds_complete_r7_through_p8v6_o8v2_lineage() -> None:
    ...
    assert tag_positions == [4, 8, 12, 16, 20, 24, 28, 32]
    for parent, child in (
...
def test_runbook_closes_exact_pre_o8_operator_and_publication_contract() -> None:
    ...
    section_one = runbook.split("## 1. Build and freeze O8v3", 1)[1].split(
    ...
    assert "a partial or failed sequence is not O8v3" in section_one
    ...
    p8v6_push = [
        ...
        (
            "refs/tags/prereg-action-qbc-v8-open-bounded-remote-verification-v6:"
            "refs/tags/prereg-action-qbc-v8-open-bounded-remote-verification-v6"
        ),
    ]
    o8v3_push = [
        ...
        "refs/heads/action-qbc-v8-prereg:refs/heads/action-qbc-v8-prereg",
        (
            "refs/tags/action-qbc-v8-open-diagnostic-freeze-v3:"
            "refs/tags/action-qbc-v8-open-diagnostic-freeze-v3"
        ),
    ]
    ...
    for argv in (p8v6_push, o8v3_push, independent_https_check):
    ...
    assert section_one.index(
        reconstruction.canonical_json_bytes(p8v6_push).decode("ascii")
    ) < section_one.index(
        reconstruction.canonical_json_bytes(o8v3_push).decode("ascii")
    ) < section_one.index(
...
def test_all_production_v7_literals_and_path_reads_are_exactly_allowlisted() -> None:
    ...
    assert hashlib.sha256(audit_source).hexdigest() == (
        "9d60cb805ddb49f10ca21a02225a9434786c24152d03054feafefb85de12256a"
    )
    audit_imports = {
        alias.name
...
def _full_preparation_ledger(
    ...
    *,
    ...
    assert len(authority_identities) == 46
