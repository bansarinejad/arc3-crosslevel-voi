...
This runbook operationalizes the frozen v8 amendment through its P8v6
runner-manifest-key recovery. The preregistration documents are authoritative when this
...
O8v1, O8v2, freeze-v1, and freeze-v2 remain immutable and may not be retried or executed.
V8 is an open public-fixture replication, not a lockbox, runtime-admission, leaderboard, or
positive-mechanism experiment.

## Immutable identities

- Binding preregistration commit: `61cebe90a2f4f7c78ec45119de53a482ed13a655`
- Binding preregistration tree: `65695876c44eeb8cac5437149384071f88ff6018`
- Preregistration tag:
...
- Preregistration document Git blob: `5e870ed0bbbff6fcb4352f6e914d870254773f68`
- Preregistration document byte count: `32370`
...
- Open-freeze tag: `action-qbc-v8-open-diagnostic-freeze-v3`
- Result branch: `action-qbc-v8-open-diagnostic-result`
- Result tag: `action-qbc-v8-open-diagnostic-result-v3`
...
Record the eventual O8v3 commit before any execution command. Every command below is a
...
## 1. Build and freeze O8v3

Perform all pre-O8v3 construction and gates in Ubuntu against the original checkout. The
...
is not embedded as one string. The operational pre-O8v3 environment is constructed from an
...
O8v3 tree, index, manifest, registration, or commit identity may depend on its bytes.
...
```json
["uv","run","--frozen","--extra","dev","python3","-I","-B","scripts/build_action_qbc_v8_open_registration.py","--repository-root",".","--preregistration-tag","prereg-action-qbc-v8-open-bounded-remote-verification-v6","--output","artifacts/action_qbc_v8_open_registration.json"]
```
...
regenerate the registration, rerun a failed array, commit O8v3, create its tag, publish refs,
bootstrap, or start any v8 process. Preserve the evidence and preregister any corrected
attempt; a partial or failed sequence is not O8v3.

Only after every check passes, commit all and only the fifteen additions as the direct child
of P8v6 and create the new lightweight
`action-qbc-v8-open-diagnostic-freeze-v3` tag at that exact O8v3 commit. The immutable
freeze-v1 and freeze-v2 tags remain at O8v1 and O8v2; reserved result-v1 and result-v2 remain
absent. The repository is already PUBLIC and must remain PUBLIC. No visibility invocation,
visibility query, or visibility mutation is authorized by this runbook.

P8v6 was published first, after its local commit audit, by this exact one-shot non-force
atomic Windows Git invocation:

```json
["C:\\Users\\User\\anaconda3\\Library\\bin\\git.exe","--no-replace-objects","--no-optional-locks","-C","D:\\kaggle competitions\\arc3-crosslevel-voi","push","--atomic","origin","refs/heads/action-qbc-v8-prereg:refs/heads/action-qbc-v8-prereg","refs/tags/prereg-action-qbc-v8-open-bounded-remote-verification-v6:refs/tags/prereg-action-qbc-v8-open-bounded-remote-verification-v6"]
```

After the exact O8v3 post-commit audit passes, use one non-force atomic push of the existing
branch and new freeze-v3 tag from the original Windows checkout:

```json
["C:\\Users\\User\\anaconda3\\Library\\bin\\git.exe","--no-replace-objects","--no-optional-locks","-C","D:\\kaggle competitions\\arc3-crosslevel-voi","push","--atomic","origin","refs/heads/action-qbc-v8-prereg:refs/heads/action-qbc-v8-prereg","refs/tags/action-qbc-v8-open-diagnostic-freeze-v3:refs/tags/action-qbc-v8-open-diagnostic-freeze-v3"]
```
...
ref query, or other network action may occur between the P8v6 push and O8v3 push or between
the O8v3 push and the following sole check. Necessary transport interaction by the two
registered pushes is not a separate query.

After successful O8v3 atomic publication, the next protocol network action is one
...
bootstrap and consumes the sole O8v3 check; it is never retried. Do not execute a v8
...
mode 0700, clone the O8v3 tag from the fixed local `file:///mnt/d/...` URL, check out detached
O8v3 as untrusted materialization, remove `origin`, install the exact closed seven-key local
...
registered `<O8_COMMIT>` placeholder with the recorded O8v3 SHA:
...
["/usr/bin/git","--no-replace-objects","clone","--no-local","--no-checkout","--branch","action-qbc-v8-open-diagnostic-freeze-v3","--single-branch","file:///mnt/d/kaggle%20competitions/arc3-crosslevel-voi","/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open/authority"]
...
Return to the original clean O8v3 Windows checkout. Confirm that all four external Windows
...
nineteen-key normalized local config, ordinary stage-zero O8v3 index, plain non-reparse admin
...
or perform another online rehearsal. If the lifecycle claim was not created, stop at O8v3 and
...
Never move, replace, or delete P8, O8v1, O8v2, O8v3, R8v3, any claim, any receipt, a bundle,
a ledger, or a tag.
...
```json
["/usr/bin/git","--no-replace-objects","-C","/mnt/d/kaggle competitions/arc3-crosslevel-voi","fetch","--no-tags","file:///var/tmp/arc3-crosslevel-voi-action-qbc-v8-open/authority","refs/tags/action-qbc-v8-open-diagnostic-result-v3:refs/heads/action-qbc-v8-open-diagnostic-result"]
["/usr/bin/git","--no-replace-objects","-C","/mnt/d/kaggle competitions/arc3-crosslevel-voi","fetch","--no-tags","file:///var/tmp/arc3-crosslevel-voi-action-qbc-v8-open/authority","refs/tags/action-qbc-v8-open-diagnostic-result-v3:refs/tags/action-qbc-v8-open-diagnostic-result-v3"]
...
["C:\\Users\\User\\anaconda3\\Library\\bin\\git.exe","--no-replace-objects","--no-optional-locks","push","origin","refs/tags/action-qbc-v8-open-diagnostic-result-v3:refs/tags/action-qbc-v8-open-diagnostic-result-v3"]
["C:\\Users\\User\\anaconda3\\Library\\bin\\git.exe","--no-replace-objects","--no-optional-locks","-c","credential.interactive=never","ls-remote","--refs","origin","refs/heads/action-qbc-v8-open-diagnostic-result","refs/tags/action-qbc-v8-open-diagnostic-result-v3"]
```
