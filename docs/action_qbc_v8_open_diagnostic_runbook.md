...
This runbook operationalizes the frozen v8 amendment through its P8v4 administrative
correction. The preregistration documents are authoritative when this summary and the
protocol differ. V8 is an open public-fixture replication, not a
lockbox, runtime-admission, leaderboard, or positive-mechanism experiment.

## Immutable identities

- Binding preregistration commit: `e0bff9ffc185196cafa938c8f7c9a7186366258b`
- Preregistration tag:
...
- Preregistration document Git blob: `29c991b7e23209f2c38d5e9a11a15bca51753d8e`
- Preregistration document byte count: `33215`
...
- Open-freeze tag: `action-qbc-v8-open-diagnostic-freeze-v1`
- Result tag: `action-qbc-v8-open-diagnostic-result-v1`
...
Record the eventual O8 commit before any execution command. Every command below is a
...
## 1. Build and freeze O8

Perform all pre-O8 construction and gates in Ubuntu against the original checkout. The
...
is not embedded as one string. The operational pre-O8 environment is constructed from an
...
O8 tree, index, manifest, registration, or commit identity may depend on its bytes.
...
```json
["uv","run","--frozen","--extra","dev","python3","-I","-B","scripts/build_action_qbc_v8_open_registration.py","--repository-root",".","--preregistration-tag","prereg-action-qbc-v8-open-bounded-remote-verification-v4","--output","artifacts/action_qbc_v8_open_registration.json"]
```
...
regenerate the registration, rerun a failed array, commit O8, create its tag, publish refs,
bootstrap, or start any v8 process. Preserve the evidence and preregister any corrected
attempt; a partial or failed sequence is not O8.

Only after every check passes, commit all and only the fifteen additions as the direct child
of P8v4 and create the new lightweight
`action-qbc-v8-open-diagnostic-freeze-v1` tag at that exact O8 commit. As an operational
publication strengthening, use one non-force atomic push of the existing branch and new tag
from the original Windows checkout:

```json
["C:\\Users\\User\\anaconda3\\Library\\bin\\git.exe","--no-replace-objects","--no-optional-locks","-C","D:\\kaggle competitions\\arc3-crosslevel-voi","push","--atomic","origin","refs/heads/action-qbc-v8-prereg:refs/heads/action-qbc-v8-prereg","refs/tags/action-qbc-v8-open-diagnostic-freeze-v1:refs/tags/action-qbc-v8-open-diagnostic-freeze-v1"]
```

Then perform one independent, credential-disabled read of the literal public HTTPS URL from
neutral WSL cwd `/mnt/d/kaggle competitions` using an empty environment:
...
refs, each once at the same recorded lowercase 40-hex O8 commit; stderr must be empty and no
...
before bootstrap; do not execute a v8 supervisor, verifier, lifecycle driver, runner,
validator, or finalizer until the exact two-ref check passes.
...
mode 0700, clone the O8 tag from the fixed local `file:///mnt/d/...` URL, check out detached
O8 as untrusted materialization, remove `origin`, install the exact closed seven-key local
...
registered `<O8_COMMIT>` placeholder with the recorded O8 SHA:
...
["/usr/bin/git","--no-replace-objects","clone","--no-local","--no-checkout","--branch","action-qbc-v8-open-diagnostic-freeze-v1","--single-branch","file:///mnt/d/kaggle%20competitions/arc3-crosslevel-voi","/var/tmp/arc3-crosslevel-voi-action-qbc-v8-open/authority"]
...
Return to the original clean O8 Windows checkout. Confirm that all four external Windows
...
nineteen-key normalized local config, ordinary stage-zero O8 index, plain non-reparse admin
...
or perform another online rehearsal. If the lifecycle claim was not created, stop at O8 and
...
Never move, replace, or delete P8, O8, R8, any claim, any receipt, a bundle, a ledger, or a
tag.
...
```json
["/usr/bin/git","--no-replace-objects","-C","/mnt/d/kaggle competitions/arc3-crosslevel-voi","fetch","--no-tags","file:///var/tmp/arc3-crosslevel-voi-action-qbc-v8-open/authority","refs/tags/action-qbc-v8-open-diagnostic-result-v1:refs/heads/action-qbc-v8-open-diagnostic-result"]
["/usr/bin/git","--no-replace-objects","-C","/mnt/d/kaggle competitions/arc3-crosslevel-voi","fetch","--no-tags","file:///var/tmp/arc3-crosslevel-voi-action-qbc-v8-open/authority","refs/tags/action-qbc-v8-open-diagnostic-result-v1:refs/tags/action-qbc-v8-open-diagnostic-result-v1"]
...
["C:\\Users\\User\\anaconda3\\Library\\bin\\git.exe","--no-replace-objects","--no-optional-locks","push","origin","refs/tags/action-qbc-v8-open-diagnostic-result-v1:refs/tags/action-qbc-v8-open-diagnostic-result-v1"]
["C:\\Users\\User\\anaconda3\\Library\\bin\\git.exe","--no-replace-objects","--no-optional-locks","-c","credential.interactive=never","ls-remote","--refs","origin","refs/heads/action-qbc-v8-open-diagnostic-result","refs/tags/action-qbc-v8-open-diagnostic-result-v1"]
```
